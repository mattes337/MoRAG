...
class StaticAnalyzer:
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if "print(" in line and not line.strip().startswith("#"):
                    warnings.append(
                        f"{filepath}:{i}: Found print statement (consider using logging)"
                    )
                ...
                if len(line) > 120:
                    warnings.append(
                        f"{filepath}:{i}: Line too long ({len(line)} > 120 characters)"
                    )
                ...
                if "shell=True" in line:
                    warnings.append(
                        f"{filepath}:{i}: Use of shell=True in subprocess (potential security risk)"
                    )
    ...
    def analyze_project(
        self, paths: List[Path], include_external_tools: bool = True
    ) -> Dict[str, Any]:
        ...
        if include_external_tools and python_files:
            ...
            if mypy_errors or mypy_warnings:
                results["file_results"].append(
                    {"file": "mypy", "errors": mypy_errors, "warnings": mypy_warnings}
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive static analysis for Python projects"
    )
    parser.add_argument(
        "paths",
        nargs="*",
    ...
    if args.json:
        ...
        for file_result in results["file_results"]:
            if file_result["errors"] or (
                file_result["warnings"] and not args.errors_only
            ):
