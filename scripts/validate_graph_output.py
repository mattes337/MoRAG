    ...
    if errors:
        ...
        if 'confidence_stats' in analysis:
            ...
            if 'entity_confidence' in conf_stats:
                ...
                print(f"   - Entity confidence: {entity_conf.get('avg', 0):.2f} (range: {entity_conf.get('min', 0):.2f}-{entity_conf.get('max', 0):.2f})")
            if 'relation_confidence' in conf_stats:
                relation_conf = conf_stats['relation_confidence']
                print(f"   - Relation confidence: {relation_conf.get('avg', 0):.2f} (range: {relation_conf.get('min', 0):.2f}-{relation_conf.get('max', 0):.2f})")
