...
class IngestionCoordinator:
    ...
    async def ingest_content(
        ...
        document_summary = await self._generate_document_summary(
            content, metadata, effective_language
        )
    ...
    async def _generate_embeddings_and_metadata(
        ...
        chunks = self._create_chunks(
            content, chunk_size, chunk_overlap, content_type, metadata
        )
        ...
        for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
            ...
            content_type = metadata.get("content_type", "document")
            chunk_meta["chunk_type"] = self._determine_chunk_type(
            ...
            if content_type in ["audio", "video"]:
                chunk_meta.update(
                    self._add_audio_video_chunk_metadata(chunk, i, metadata)
                )
    ...
    def _create_chunks(
        ...
        if content_type in ["audio", "video"] and metadata:
            return self._create_topic_based_chunks(
        ...
        # Use function/class boundary chunking for code files
            ...
            return self._create_code_structural_chunks(
    ...
    def _create_topic_based_chunks(
        self,
        content: str,
        chunk_size: int,
        chunk_overlap: int,
        ...
        for i, match in enumerate(topic_matches):
            ...
            if len(topic_content) > chunk_size:
                ...
                sub_chunks = self._split_topic_at_timestamps(
    ...
    def _create_timestamp_chunks(
        ...
        timestamp_matches = list(
            re.finditer(timestamp_pattern, content, re.MULTILINE | re.DOTALL)
        )
        ...
        for match in timestamp_matches:
            ...
            if (
                current_chunk and len(current_chunk) + len(line) + 1 > chunk_size
            ):  # +1 for newline
    ...
    def _split_topic_at_timestamps(
        ...
        timestamp_chunks = self._create_timestamp_chunks(
            remaining_content, chunk_size - len(topic_header), chunk_overlap
        )
    ...
    def _create_image_section_chunks(
        self, content: str, chunk_size: int, chunk_overlap: int
    ) -> List[str]:
        ...
        for i, section in enumerate(sections):
            ...
            if (
                current_chunk
                and len(current_chunk) + len(full_section) + 2 > chunk_size
            ):
        ...
        for chunk in chunks:
            if len(chunk) > chunk_size:
                ...
                sub_chunks = self._create_character_chunks(
    ...
    def _create_web_article_chunks(
        self, content: str, chunk_size: int, chunk_overlap: int
    ) -> List[str]:
    ...
    def _create_text_semantic_chunks(
        self, content: str, chunk_size: int, chunk_overlap: int
    ) -> List[str]:
    ...
    def _create_code_structural_chunks(
        self, content: str, chunk_size: int, chunk_overlap: int
    ) -> List[str]:
        ...
        for line in lines:
            ...
            is_function_start = any(
                re.match(pattern, line) for pattern in function_patterns
            )
            ...
            if is_function_start:
                ...
                if (
                    current_chunk
                    and len(current_chunk) + len(current_function) > chunk_size
                ):
                    if current_chunk.strip():
                        chunks.append(current_chunk.strip())
    ...
    def _create_archive_file_chunks(
        self, content: str, chunk_size: int, chunk_overlap: int
    ) -> List[str]:
        ...
        for i in range(0, len(sections), 2):
            if i + 1 < len(sections):
                ...
                if (
                    current_chunk
                    and len(current_chunk) + len(full_file) + 2 > chunk_size
                ):
                    # Save current chunk and start new one
                    if current_chunk.strip():
                        chunks.append(current_chunk.strip())
    ...
    async def _extract_graph_data(
        ...
        try:
            ...
            for i, chunk_content in enumerate(chunks):
                try:
                    logger.debug(
                        f"Processing chunk {i+1}/{len(chunks)} for fact extraction"
                    )
                    ...
                    logger.debug(
                        f"Chunk {i+1} processed: {len(chunk_facts)} facts, {len(chunk_relationships)} relationships"
                    )

                except Exception as e:
                    logger.warning(
                        f"Failed to process chunk {i+1} for fact extraction",
                        error=str(e),
                    )
            ...
            logger.info(
                f"Extracted {len(all_facts)} facts and {len(all_relationships)} relationships from {len(chunks)} chunks"
            )
            logger.info(
                f"Created chunk-fact mapping: {len(chunk_fact_mapping)} chunks with facts"
            )
            ...
            (
                entity_embeddings,
                fact_embeddings,
            ) = await self._generate_entity_and_fact_embeddings(
    ...
    async def _generate_entity_and_fact_embeddings(
        self, facts: List[Dict[str, Any]], relationships: List[Dict[str, Any]]
        ...
        try:
            ...
            for fact in facts:
                ...
                if fact.get("keywords"):
                    keywords = (
                        fact["keywords"].split(",")
                        if isinstance(fact["keywords"], str)
                        else fact["keywords"]
                    )
            ...
            logger.info(
                f"Found {len(unique_entities)} unique entities for embedding generation"
            )

            # Generate embeddings for entities in batches
            if unique_entities:
                entity_embeddings = await self._generate_entity_embeddings_batch(
                    unique_entities
                )
            ...
            logger.info(
                f"Generated embeddings: {len(entity_embeddings)} entities, {len(fact_embeddings)} facts"
            )
    ...
    async def _generate_entity_embeddings_batch(
        self, entities: Dict[str, Dict[str, Any]], batch_size: int = 50
    ) -> Dict[str, Any]:
        """Generate embeddings for entities in batches.
        ...
        try:
            for i in range(0, len(entity_list), batch_size):
                ...
                logger.info(
                    f"Processing entity embedding batch {i//batch_size + 1}: {len(batch)} entities"
                )
                ...
                try:
                    ...
                    for j, (entity_key, entity_data) in enumerate(batch):
                        if j < len(batch_embeddings):
                            ...
                            if hasattr(embedding, "embedding"):
                                ...
                                logger.warning(
                                    f"Unexpected embedding type: {type(embedding)}"
                                )
                                continue
                    ...
                    logger.warning(
                        f"Failed to generate embeddings for entity batch {i//batch_size + 1}: {e}"
                    )
                    ...
                    for entity_key, entity_data in batch:
                        try:
                            ...
                            logger.warning(
                                f"Failed to generate embedding for entity {entity_key}: {individual_e}"
                            )
    ...
    async def _generate_fact_embeddings_batch(
        self, facts: List[Dict[str, Any]], batch_size: int = 50
    ) -> Dict[str, Any]:
        """Generate embeddings for facts in batches.
        ...
        try:
            for i in range(0, len(facts), batch_size):
                ...
                logger.info(
                    f"Processing fact embedding batch {i//batch_size + 1}: {len(batch)} facts"
                )
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to generate embeddings for fact batch {i//batch_size + 1}: {e}"
                    )
                    # Try individual embeddings as fallback
                    for fact in batch:
                        try:
                            ...
                            logger.warning(
                                f"Failed to generate embedding for fact {fact['id']}: {individual_e}"
                            )
    ...
    async def _process_facts_with_enhanced_processing(
        ...
        try:
            ...
            for fact_dict in facts:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to convert fact dictionary to object: {e}"
                    )
                    continue
            ...
            if api_key:
                ...
                logger.warning(
                    "GEMINI_API_KEY not found - entity normalization disabled"
                )

            enhanced_processor = EnhancedFactProcessingService(
                neo4j_storage, entity_normalizer
            )
    ...
    def _create_chunk_entity_mapping(
        self, content: str, entities: List[Entity], chunk_size: int, chunk_overlap: int
    ) -> Dict[str, List[str]]:
        ...
        for chunk_index, chunk_text in enumerate(chunks):
            ...
            if entities_in_chunk:
                ...
                logger.debug(
                    f"Chunk {chunk_index} contains {len(entities_in_chunk)} entities"
                )

        logger.info(
            f"Found entities in {len(chunk_entity_mapping)} out of {len(chunks)} chunks"
        )
    ...
    def _enhance_chunk_entity_mapping_with_missing_entities(
        ...
        logger.info(
            f"Found {len(auto_created_entities)} auto-created entities to map to chunks"
        )
            ...
            for chunk_index, chunk_text in enumerate(chunks):
                ...
                if entity_name_lower in chunk_text_lower:
                    ...
                    if entity_id not in chunk_entity_mapping[chunk_index_str]:
                        ...
                        logger.debug(
                            f"Added auto-created entity '{entity_name}' ({entity_id}) to chunk {chunk_index}"
                        )
        ...
        if unconnected_entities:
            logger.info(
                f"Found {len(unconnected_entities)} unconnected auto-created entities, connecting via relations"
            )
            ...
            for entity_name, entity_id in unconnected_entities:
                        ...
                        for (
                            ...
                            if related_entity_id in chunk_entities:
                                ...
                                if entity_id not in chunk_entities:
                                    ...
                                    logger.debug(
                                        f"Connected auto-created entity '{entity_name}' ({entity_id}) to chunk {chunk_index_str} via related entity {related_entity_id}"
                                    )
                                break
                ...
                if not connected_to_chunk and chunk_entity_mapping:
                    ...
                    entities_added_to_chunks += 1
                    logger.warning(
                        f"Connected auto-created entity '{entity_name}' ({entity_id}) to first chunk {first_chunk} as fallback"
                    )

        logger.info(
            f"Enhanced chunk-entity mapping: added {entities_added_to_chunks} auto-created entity mappings"
        )
        return chunk_entity_mapping
    ...
    def _create_ingest_result(
        ...
        return {
            ...
            "embeddings_data": {
                ...
                "embedding_dimension": len(embeddings_data["embeddings"][0])
                ...
                "chunks": [
                    {
                        ...
                        "metadata": {
                            k: v for k, v in meta.items() if k not in ["chunk_text"]
                        },
                    }
                    for meta, embedding, chunk_text in zip(
                        embeddings_data["chunk_metadata"],
    ...
    def _write_ingest_result_file(
        self, source_path: str, ingest_result: Dict[str, Any]
    ) -> str:
        """Write the ingest_result.json file."""
        source_path_obj = Path(source_path)
        result_file_path = (
            source_path_obj.parent / f"{source_path_obj.stem}.ingest_result.json"
        )
    ...
    def _create_ingest_data(
        ...
        if embeddings_data["chunk_metadata"]:
            ...
            document_metadata = {
                ...
                "name": first_chunk_meta.get(
                    "source_name",
                    first_chunk_meta.get("file_name", source_path_obj.name),
                ),
                "mime_type": first_chunk_meta.get("mime_type", "unknown"),
                "file_size": first_chunk_meta.get("file_size"),
                "checksum": first_chunk_meta.get(
                    "checksum", first_chunk_meta.get("content_checksum")
                ),
        ...
        return {
            ...
            "graph_data": {
                ...
                "enhanced_processing": self._serialize_enhanced_processing(
                    graph_data.get("enhanced_processing", {})
                ),
    ...
    def _write_ingest_data_file(
        self, source_path: str, ingest_data: Dict[str, Any]
    ) -> str:
        """Write the ingest_data.json file."""
        source_path_obj = Path(source_path)
        data_file_path = (
            source_path_obj.parent / f"{source_path_obj.stem}.ingest_data.json"
        )
    ...
    async def _initialize_databases(
        self, database_configs: List[DatabaseConfig], embeddings_data: Dict[str, Any]
    ) -> None:
    ...
    async def _initialize_qdrant(
        self, db_config: DatabaseConfig, embeddings_data: Dict[str, Any]
    ) -> None:
        ...
        logger.info(
            "Initializing Qdrant with config",
        ...
        logger.info(
            "Creating QdrantVectorStorage with",
    ...
    async def _store_enhanced_processing_data(
        self, neo4j_storage, graph_data: Dict[str, Any], facts: List
    ):
        ...
        try:
            ...
            # Store facts first
            if facts:
                from morag_graph.storage.neo4j_operations.fact_operations import (
                    FactOperations,
                )

                fact_operations = FactOperations(
                    neo4j_storage.driver, neo4j_storage.config.database
                )
            ...
            if entities:
                from morag_graph.models.entity import Entity
                from morag_graph.storage.neo4j_operations.entity_operations import (
                    EntityOperations,
                )
                ...
                logger.info(
                    f"Stored {len(entities)} entities from enhanced processing data"
                )
                        ...
                        if (
                            ...
                            for entity in entities:
                                ...
                                if name_match and type_match:
                                    ...
                                    logger.debug(
                                        f"Found entity match: {entity.name} ({entity.type}) -> {actual_entity_id}"
                                    )
                                    break

                            if actual_entity_id:
                                await entity_embedding_service.store_entity_embedding(
                                    actual_entity_id, embedding_vector
                                )
                                logger.debug(
                                    f"Stored embedding for entity {actual_entity_id} (name: {entity_name})"
                                )
                            else:
                                # Enhanced logging to debug the issue
                                logger.warning(
                                    f"Could not find actual entity ID for entity key {entity_key} (name: {entity_name}, type: {entity_type})"
                                )

                                # Show more entities for debugging and check for partial matches
                                available_entities = [
                                    (e.name, str(e.type)) for e in entities[:10]
                                ]
                                logger.debug(
                                    f"Available entities (first 10): {available_entities}"
                                )
                                ...
                                partial_matches = [
                                    ...
                                    or e.name.lower() in entity_name.lower()
                                ]
                                if partial_matches:
                                    logger.debug(
                                        f"Potential partial name matches: {partial_matches[:5]}"
                                    )

                                logger.debug(
                                    f"Total entities available: {len(entities)}, searching for: '{entity_name}' (type: {entity_type})"
                                )
                        else:
                            logger.warning(
                                f"Invalid embedding data format for entity {entity_key}: {type(embedding_data)}"
                            )
                            continue
            ...
            if fact_embeddings:
                ...
                from morag_graph.services.fact_embedding_service import (
                ...
                if api_key:
                    ...
                    for fact_id, embedding_data in fact_embeddings.items():
                        ...
                        if (
                            ...
                            embedding_vector = embedding_data["embedding"]
                            await fact_embedding_service.store_fact_embedding(
                                fact_id, embedding_vector
                            )
                        else:
                            logger.warning(
                                f"Invalid embedding data format for fact {fact_id}: {type(embedding_data)}"
                            )
                            continue
        ...
        for db_config in database_configs:
            try:
                if db_config.type == DatabaseType.QDRANT:
                    ...
                    document_metadata = (
                        ingest_data.get("document_metadata", {}) if ingest_data else {}
                    )
    ...
    async def _write_to_neo4j(
        ...
        try:
            ...
            if document_metadata:
                ...
                summary = document_summary or document_metadata.get(
                ...
                mime_type = chunk_meta.get(
                    "mime_type", chunk_meta.get("source_type", "unknown")
                )
                file_size = chunk_meta.get("file_size")
                checksum = chunk_meta.get("checksum") or chunk_meta.get(
                    "content_checksum"
                )
                summary = document_summary or chunk_meta.get(
                    "summary", "Document processed successfully"
                )
                metadata = chunk_meta
            ...
            chunk_id_to_index = (
                {}
            )  # Map chunk_id to chunk_index for entity relationships
            ...
            if facts_to_process:
                ...
                if api_key:
                    ...
                    language = (
                        facts_to_process[0].language
                        if facts_to_process and hasattr(facts_to_process[0], "language")
                        else "en"
                    )
                    entity_normalizer = LLMEntityNormalizer(
                        model_name=os.getenv("MORAG_GEMINI_MODEL", "gemini-2.0-flash"),
                ...
                logger.info(
                    f"Enhanced processing: {result['facts_processed']} facts, {result['entities_created']} entities, {result['relations_created']} relations"
                )
            ...
            if fact_relationships:
                ...
                relationship_ids = await fact_operations.store_fact_relations(
                    fact_relationships
                )
            ...
            logger.info(
                f"Creating chunk-fact relationships: {len(chunk_fact_mapping)} chunks with facts, {len(chunk_ids)} total chunks"
            )
            ...
            for chunk_key, fact_ids_in_chunk in chunk_fact_mapping.items():
                ...
                if ":chunk:" in chunk_key:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not parse chunk key '{chunk_key}' as index or ID"
                        )
                        continue
                ...
                if chunk_id:
                    ...
                    chunk_text = (
                        embeddings_data["chunks"][chunk_index]
                        if chunk_index < len(embeddings_data["chunks"])
                        else ""
                    )
                    context = (
                        chunk_text[:500] + "..."
                    ...
                    logger.debug(
                        f"Creating relationships for chunk {chunk_index} (id: {chunk_id}) with {len(fact_ids_in_chunk)} facts"
                    )

                    # Create chunk -> CONTAINS -> fact relationships
                    for fact_id in fact_ids_in_chunk:
                        try:
                            await neo4j_storage.create_chunk_contains_fact_relation(
                                chunk_id, fact_id, context
                            )
                            chunk_fact_relationships_created += 1
                            logger.debug(
                                f"Created relationship: chunk {chunk_id} -> fact {fact_id}"
                            )

                        except Exception as e:
                            logger.warning(
                                f"Failed to create chunk-fact relationship: chunk {chunk_id} -> fact {fact_id}: {e}"
                            )
                else:
                    logger.warning(
                        f"Could not find chunk_id for chunk_key '{chunk_key}' (parsed as chunk_index {chunk_index}). Available chunks: {list(chunk_id_to_index.values())}"
                    )

            logger.info(
                f"Created {chunk_fact_relationships_created} chunk-fact relationships"
            )

            # Note: Fact storage and relationship creation will be implemented when Neo4j fact storage is added
            logger.info(
                f"Neo4j storage completed: {len(chunk_ids)} chunks stored, {facts_stored} facts processed, {relationships_stored} relationships processed"
            )
    ...
    async def _generate_document_summary(
        self, content: str, metadata: Dict[str, Any], language: Optional[str] = None
    ) -> str:
        ...
        try:
            ...
            summary_result = await self.embedding_service.generate_summary(
                content, max_length=max_length, language=language
            )
    ...
    def _determine_chunk_type(self, content_type: str, metadata: Dict[str, Any]) -> str:
        ...
        if content_type in ["audio", "video"]:
            ...
            if (
                metadata.get("has_topic_info", False)
                or metadata.get("num_topics", 0) > 1
            ):
                return "topic"
            else:
                return "segment"
    ...
    def _add_audio_video_chunk_metadata(
        self, chunk_text: str, chunk_index: int, metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if segments and chunk_index < len(segments):
            ...
            if isinstance(segment, dict):
                ...
                if "start_time" in segment:
                    ...
                    additional_meta["start_timestamp"] = self._seconds_to_timestamp(
                        segment["start_time"]
                    )
                if "end_time" in segment:
                    additional_meta["end_timestamp_seconds"] = segment["end_time"]
                    additional_meta["end_timestamp"] = self._seconds_to_timestamp(
                        segment["end_time"]
                    )
        ...
        if "start_timestamp_seconds" not in additional_meta:
            ...
            timestamp_pattern = (
                r"\[(\d{2}:\d{2}:\d{2})\]|\[(\d+:\d{2})\]|(\d+:\d{2}:\d{2})|(\d+:\d{2})"
            )
        ...
        if metadata.get("has_topic_info", False):
            ...
            if topic_match:
                ...
                additional_meta[
                    "chunk_summary"
                ] = f"Topic: {topic_match.group(1).strip()}"
        ...
        if "start_timestamp_seconds" in additional_meta:
            timestamp_str = additional_meta.get(
                "start_timestamp",
                self._seconds_to_timestamp(additional_meta["start_timestamp_seconds"]),
            )
    ...
    def _add_document_chunk_metadata(
        self, chunk_text: str, chunk_index: int, metadata: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if page_number is None:
            ...
            for pattern in page_patterns:
                page_match = re.search(
                    pattern, chunk_text, re.MULTILINE | re.IGNORECASE
                )
                if page_match:
                    try:
        ...
        if page_number is None and "page_count" in metadata and metadata["page_count"]:
            ...
            if total_chunks > 0 and page_count > 0:
                estimated_page = max(
                    1, int((chunk_index / total_chunks) * page_count) + 1
                )
...
class FactExtractionWrapper:
    ...
    async def extract_facts_and_relationships(
        ...
        try:
            # Extract facts
            logger.debug(
                "Extracting facts from text", text_length=len(text), doc_id=doc_id
            )
