...
class MoRAGServices:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(
                "Audio service not available, audio processing disabled", error=str(e)
            )
        ...
        logger.info(
            "MoRAG Services initialized",
            graph_enabled=self.graph_processor.is_enabled(),
        )
    ...
    def _initialize_search_services(self):
        ...
        try:
            ...
            if llm_config.api_key:
                ...
                logger.warning(
                    "API key not found - search functionality will be limited"
                )
    ...
    def _is_youtube(self, path_or_url: str) -> bool:
        ...
        return path_or_url.startswith("http") and any(
            domain in path_or_url for domain in youtube_domains
        )
        ...
        try:
            ...
            json_result = await self.document_service.process_document_to_json(
                ...
                **{
                    ...
                    if k
                    not in ["progress_callback", "databases", "enable_graph_processing"]
                },
            )
                ...
                try:
                    ...
                    if database_configs:
                        ...
                        if self.graph_processor:
                            graph_result = (
                                await self.graph_processor.process_document_multi_db(
                    ...
                    if graph_result:
                        logger.info(
                            ...
                            databases_processed=len(graph_result.database_results)
                            if hasattr(graph_result, "database_results")
                            else 1,
                        )

                except Exception as e:
                    logger.warning(
                        "Graph processing failed, continuing with document processing",
                        document_path=document_path,
                        error=str(e),
                    )
            ...
            processing_result = ProcessingResult(
                ...
                processing_time=json_result.get("metadata", {}).get(
                    "processing_time", 0.0
                ),
                success=True,
            ...
            if self._data_writer and data_file_path and text_content:
                try:
                    ...
                    logger.warning(
                        f"Failed to update data file with processing result: {str(e)}"
                    )
    ...
    async def process_video(
        ...
        try:
            ...
            markdown_result = await self.video_service.convert_result_to_markdown(
                json_result
            )
            ...
            return ProcessingResult(
                ...
                metadata=json_result.get("metadata", {}),
                extracted_files=(json_result.get("thumbnails", []) or [])
                + (json_result.get("keyframes", []) or []),
                processing_time=json_result.get("processing_time", 0.0),
                success=markdown_result.get("success", False)
                or json_result.get("success", False),
    ...
    async def process_url(
        ...
        try:
            result = await self.web_service.process_url(
                url,
                config_options=self.config.web_config.to_dict()
                if self.config.web_config and hasattr(self.config.web_config, "to_dict")
                else None,
            )
        ...
        try:
            ...
            if options:
                ...
                if options.get("pre_transcribed", False):
                    ...
                    youtube_config.provided_transcript_segments = options.get(
                        "transcript_segments"
                    )
                else:
                    # Configure Apify options
                    youtube_config.extract_metadata = options.get(
                        "extract_metadata", True
                    )
                    youtube_config.extract_transcript = options.get(
            ...
            result = await self.youtube_service.transcribe_video(
                ...
                transcript_segments=options.get("transcript_segments")
                if options
                else None,
            )
    ...
    async def generate_embeddings(
        ...
        if isinstance(text, list):
            return await self.embedding_service.generate_batch_embeddings(
                text, config=self.config.embedding_config
            )
        else:
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        for service_name, service in services_to_check:
            try:
                ...
                logger.error(
                    f"Health check failed for {service_name} service", error=str(e)
                )
        ...
        if unhealthy_services:
            health_status["overall_status"] = (
                "degraded"
                if len(unhealthy_services) < len(services_to_check)
                else "unhealthy"
            )
    ...
    async def search_similar(
        ...
        try:
            ...
            if not self._vector_storage or not self._gemini_embedding_service:
                logger.warning(
                    "Search services not available - initializing embedding service or vector storage failed"
                )
                return []
            ...
            for result in results:
                ...
                clean_metadata = {
                    k: v for k, v in result.get("metadata", {}).items() if k != "text"
                }
    ...
    def _create_youtube_comprehensive_metadata(
        self, url: str, result
    ) -> Dict[str, Any]:
        ...
        metadata = {
            ...
            "transcript_segments_count": len(
                result.transcript.get("segments", [])
                if isinstance(result.transcript, dict)
                else []
            ),
            "has_transcript": bool(result.transcript),
            "processing_method": "apify"
            if not getattr(result, "pre_transcribed", False)
            else "pre_transcribed",
        }
