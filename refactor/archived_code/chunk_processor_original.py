...
class ChunkProcessor:
            ...
            if len(current_chunk) + len(topic) + 2 <= chunk_size:
                ...
                if len(topic) > chunk_size:
                    topic_chunks = self._split_topic_at_timestamps(
                        topic, chunk_size, chunk_overlap
                    )
    ...
    def _create_timestamp_chunks(
        ...
        logger.info(
            "Creating timestamp-based chunks",
            chunk_size=chunk_size,
            chunk_overlap=chunk_overlap,
        )
        ...
        for segment in segments:
            ...
            if len(current_chunk) + len(segment) + 2 <= chunk_size:
                ...
                if len(segment) > chunk_size:
                    # Split large segment
                    segment_chunks = self._create_character_chunks(
                        segment, chunk_size - len(overlap_text), chunk_overlap
                    )
                    current_chunk = (
                        overlap_text + "\n\n" + segment_chunks[0]
                        if overlap_text
                        else segment_chunks[0]
                    )
        ...
        for section in sections:
            ...
            if len(current_chunk) + len(section) + 2 <= chunk_size:
                ...
                if len(section) > chunk_size:
                    ...
                    section_chunks = self._create_character_chunks(
    ...
    def _create_web_article_chunks(
        ...
        for pattern in section_patterns:
            try:
                ...
                chunks = self._process_sections_into_chunks(
                ...
                if chunks:
                    ...
                    variance = sum((size - avg_size) ** 2 for size in sizes) / len(
                        sizes
                    )
        ...
        if not best_chunks:
            best_chunks = self._create_text_semantic_chunks(
                content, chunk_size, chunk_overlap
            )
                    ...
                    for sentence in sentences:
                        if len(sentence_chunk) + len(sentence) + 1 <= chunk_size:
                            ...
                            if len(sentence) > chunk_size:
                                ...
                                clause_chunks = self._create_character_chunks(
                                    sentence, chunk_size, chunk_overlap
                                )
                                chunks.extend(clause_chunks[:-1])
                                sentence_chunk = (
                                    clause_chunks[-1] if clause_chunks else ""
                                )
                            else:
    ...
    def _create_code_structural_chunks(
        ...
        logger.info(
            "Creating code structural chunks",
            chunk_size=chunk_size,
            chunk_overlap=chunk_overlap,
        )
            ...
            for line in lines:
                if len(current_chunk) + len(line) + 1 <= chunk_size:
                    ...
                    if len(line) > chunk_size:
                        line_chunks = self._create_character_chunks(
                            line, chunk_size, chunk_overlap
                        )
    ...
    def _create_character_chunks(
        ...
        logger.info(
            "Creating character-based chunks",
    ...
    async def generate_embeddings_and_metadata(
        ...
        try:
            ...
            if self.embedding_service and content.strip():
                ...
                content_embedding = (
                    content_embeddings[0] if content_embeddings else None
                )
            ...
            embeddings_data = {
                ...
                "summary": summary,
                "embedding_dimension": len(chunk_embeddings[0])
                if chunk_embeddings
                else 768,
        ...
        if metadata.get("topics"):
            ...
            for topic in metadata["topics"]:
                if any(
                    ...
                    for keyword in topic.lower().split()[:3]
                ):
    ...
    def _add_document_chunk_metadata(
        ...
        for pattern, heading_type in heading_patterns:
            ...
            for match in matches:
                headings.append({"text": match, "type": heading_type})
        ...
        try:
            ...
            if len(non_empty_lines) < 5:
                return (
                    non_empty_lines[0]
                    if non_empty_lines
                    else "Content summary unavailable"
                )
