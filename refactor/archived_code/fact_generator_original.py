...
if TYPE_CHECKING:
    ...
    from morag_graph.extraction import EntityNormalizer, FactExtractor

try:
    from morag_core.ai import AgentConfig as _AgentConfig
    from morag_core.ai import create_agent as _create_agent
    from morag_graph.extraction import EntityNormalizer as _EntityNormalizer
    from morag_graph.extraction import FactExtractor as _FactExtractor
...
class FactGeneratorStage(Stage):
            ...
            if api_key:
                self.fact_extractor = FactExtractor(
                    ...
                    min_confidence=float(
                        os.getenv("MORAG_FACT_GENERATOR_MIN_CONFIDENCE", "0.3")
                    ),
                    allow_vague_language=os.getenv(
                        "MORAG_FACT_GENERATOR_ALLOW_VAGUE_LANGUAGE", "true"
                    ).lower()
                    == "true",
                    require_entities=os.getenv(
                        "MORAG_FACT_GENERATOR_REQUIRE_ENTITIES", "false"
                    ).lower()
                    == "true",
                    min_fact_length=int(
                        os.getenv("MORAG_FACT_GENERATOR_MIN_FACT_LENGTH", "20")
                    ),
                    strict_validation=os.getenv(
    ...
    async def _initialize_services_with_config(
        self, config: FactGeneratorConfig, context: "StageContext"
    ):
        ...
        if hasattr(context, "config") and context.config:
            ...
            if model_config:
                ...
                fact_extraction_model = model_config.get("agent_models", {}).get(
                    "fact_extraction"
                )
                if fact_extraction_model:
                    model_id = fact_extraction_model
                    logger.info(
                        f"Using fact extraction agent model override: {model_id}"
                    )
        ...
        if EntityNormalizer is not None:
            self.entity_normalizer = EntityNormalizer(
                model_name=model_id, api_key=api_key
            )
    ...
    async def execute(
        ...
        logger.info(
            "Starting fact generation", input_file=str(input_file), config=config
        )
        ...
        try:
            ...
            output_file = (
                context.output_dir
                / f"{input_file.stem.replace('.chunks', '')}.facts.json"
            )
            ...
            logger.info(
                "Processing chunks for fact extraction",
            ...
            for i, chunk in enumerate(chunks):
                logger.debug(
                    "Processing chunk for fact extraction",
            ...
            logger.error(
                "Fact generation failed", input_file=str(input_file), error=str(e)
            )
        ...
        if self.fact_extractor and SERVICES_AVAILABLE:
            try:
                ...
                if hasattr(config, "min_confidence"):
                    self.fact_extractor.min_confidence = getattr(
                        config, "min_confidence", 0.5
                    )
                    self.fact_extractor.validator.min_confidence = getattr(
                        config, "min_confidence", 0.5
                    )

                if hasattr(config, "allow_vague_language"):
                    self.fact_extractor.validator.allow_vague_language = getattr(
                        config, "allow_vague_language", False
                    )

                if hasattr(config, "require_entities"):
                    self.fact_extractor.validator.require_entities = getattr(
                        config, "require_entities", True
                    )

                if hasattr(config, "min_fact_length"):
                    self.fact_extractor.validator.min_fact_length = getattr(
                        config, "min_fact_length", 20
                    )

                if hasattr(config, "strict_validation"):
                    self.fact_extractor.validator.strict_validation = getattr(
                ...
                logger.debug(
                    "Fact extractor returned results",
                    chunk_id=chunk_id,
                ...
                for fact in facts:
                    fact_dict = {
                        ...
                        "structured_metadata": fact.structured_metadata.model_dump()
                        if hasattr(fact.structured_metadata, "model_dump")
                        else fact.structured_metadata,
                    }
                    ...
                    if (
                        ...
                        for entity_name in primary_entities:
                            entity_id = (
                                f"entity_{hash(entity_name.lower()) % 100000:05d}"
                            )
                            if entity_id not in entities_dict:
                                ...
                                if (
                                    chunk_id
                                    not in entities_dict[entity_id]["source_chunks"]
                                ):
                                    entities_dict[entity_id]["source_chunks"].append(
                                        chunk_id
                                    )
                                if (
                                    fact.id
                                    not in entities_dict[entity_id]["source_facts"]
                                ):
                                    entities_dict[entity_id]["source_facts"].append(
                                        fact.id
                                    )
                        ...
                        for concept_name in domain_concepts:
                            entity_id = (
                                f"entity_{hash(concept_name.lower()) % 100000:05d}"
                            )
                            if entity_id not in entities_dict:
                                entities_dict[entity_id] = {
                                    "id": entity_id,
                                    ...
                                    "type": "CONCEPT",
                                    "confidence": fact.extraction_confidence
                                    * 0.8,  # Slightly lower confidence for concepts
                                    "source_chunks": [chunk_id],
                                    "source_facts": [fact.id],
                                    "domain": fact.domain,
                                    "language": fact.language,
                        ...
                        logger.debug(
                            f"Processing relationships for fact {fact.id}: {len(relationships)} relationships found"
                        )
                        for rel in relationships:
                            logger.debug(
                                f"Processing relationship: {rel} (type: {type(rel)})"
                            )
                            ...
                            if source and rel_type and target:
                                logger.debug(
                                    f"Relationship validation passed: {source} --[{rel_type}]--> {target}"
                                )
                                ...
                                if existing_relation:
                                    ...
                                    if (
                                        chunk_id
                                        not in existing_relation["source_chunks"]
                                    ):
                                        existing_relation["source_chunks"].append(
                                            chunk_id
                                        )
                                    if fact.id not in existing_relation["source_facts"]:
                                        existing_relation["source_facts"].append(
                                            fact.id
                                        )
                                else:
                                    # Create new relation
                                ...
                                logger.debug(
                                    f"Relationship validation failed: source={source}, type={rel_type}, target={target}"
                                )

                logger.debug(
                    f"Final relations_list before creating extraction_result: {len(relations_list)} relations"
                )
                for i, rel in enumerate(relations_list):
                    logger.debug(
                        f"Relation {i}: {rel.get('source', 'N/A')} --[{rel.get('type', 'N/A')}]--> {rel.get('target', 'N/A')}"
                    )
                ...
                logger.debug(
                    f"Extraction result relations count: {len(extraction_result.get('relations', []))}"
                )
                ...
                logger.warning(
                    "Fact extractor failed, using LLM fallback",
                    error=str(e),
                    chunk_id=chunk_id,
            ...
            logger.debug(
                "Using LLM fallback for fact extraction",
                chunk_id=chunk_id,
                reason="fact_extractor_not_available"
                if not self.fact_extractor
                else "services_not_available",
            )
            results = await self._llm_extraction_fallback(content, chunk_id, config)
    ...
    def _filter_metadata_content(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if line.startswith("## ") and any(
                keyword in line_lower
                for keyword in ["content", "transcript", "text", "body", "summary"]
            ):
                skip_section = False
                continue
        ...
        if (
            original_length > 0
            and (original_length - filtered_length) / original_length > 0.1
        ):
            logger.debug(
                "Filtered metadata from content",
                original_length=original_length,
                filtered_length=filtered_length,
                reduction_percent=round(
                    (original_length - filtered_length) / original_length * 100, 1
                ),
            )
    ...
    async def _llm_extraction_fallback(
        ...
        logger.debug(
            "Starting LLM extraction fallback",
            chunk_id=chunk_id,
            content_length=len(content),
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if not api_key:
                        logger.warning(
                            "No Google AI API key found, cannot use LLM fallback"
                        )
                        return {
                            "entities": [],
                            "relations": [],
                    ...
                    logger.warning(
                        "Neither morag_reasoning nor google.generativeai available, cannot use LLM fallback"
                    )
                    return {
                        "entities": [],
                        "relations": [],
            ...
            try:
                ...
                if PARSER_AVAILABLE and LLMResponseParser:
                    ...
                    try:
                        ...
                        logger.debug(
                            "Successfully parsed JSON directly", chunk_id=chunk_id
                        )
                ...
                if not isinstance(results, dict):
                    logger.warning(
                        "LLM response is not a dict, using fallback",
                        chunk_id=chunk_id,
    ...
    async def _normalize_entities(
        ...
        try:
            ...
            normalized_variations = (
                await self.entity_normalizer.normalize_entities_batch(
            ...
            for i, entity in enumerate(entities):
                if i < len(normalized_variations):
                    ...
                    ] = normalized_variations[i].confidence
        ...
        for relation in relations:
            ...
            if relation_key not in seen_relations:
                ...
                for existing in deduplicated:
                    if (
                        existing.get("source", "").lower() == source
                        and existing.get("type", "").upper() == rel_type
                        and existing.get("target", "").lower() == target
                    ):
                        # Merge source chunks
