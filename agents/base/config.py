...
class ModelConfig(BaseModel):
    ...
    provider: ModelProvider = Field(
        default=ModelProvider.GEMINI, description="LLM provider"
    )
...
class RetryConfig(BaseModel):
    ...
    max_delay: float = Field(
        default=60.0, ge=0.0, description="Maximum delay between retries"
    )
...
class PromptConfig(BaseModel):
    ...
    include_context: bool = Field(
        default=True, description="Include contextual information"
    )
    include_instructions: bool = Field(
        default=True, description="Include detailed instructions"
    )
    ...
    custom_examples: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Custom examples"
    )
    ...
    max_output_length: Optional[int] = Field(
        default=None, ge=1, description="Maximum output length"
    )
