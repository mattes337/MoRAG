...
class EntityDeduplicationCLI:
    ...
    async def setup_services(
        ...
        try:
            ...
            if os.getenv("GEMINI_API_KEY"):
                ...
                print(
                    "⚠️  GEMINI_API_KEY not found. Using rule-based normalization only."
                )
    ...
    async def run_deduplication(
        ...
        try:
            ...
            print(
                "   For full deduplication, use the maintenance jobs or graph extraction pipeline."
            )
    ...
    async def show_candidates(
        self, collection_name: Optional[str] = None, language: Optional[str] = None
    ) -> None:
...
def print_results(result: Dict[str, Any], dry_run: bool = False):
    ...
    if dry_run:
        print(
            f"   Merges that would be applied: {len([c for c in result.get('candidates', []) if c['confidence'] >= 0.8])}"
        )
        print()
        ...
        for candidate in result.get("candidates", []):
            if candidate["confidence"] >= 0.8:
                print(
                    f"   • {', '.join(candidate['entities'])} → {candidate['canonical_form']} (confidence: {candidate['confidence']:.2f})"
                )
    else:
        ...
        if result["merge_results"]:
            ...
            for merge in result["merge_results"]:
                print(
                    f"   • {', '.join(merge['merged_entities'])} → {merge['canonical_form']} (confidence: {merge['confidence']:.2f})"
                )
...
Environment variables:
  ...
  GEMINI_API_KEY     Google Gemini API key for enhanced normalization (optional)
    ...
    parser.add_argument(
        "--collection", "-c", help="Collection name to deduplicate (optional)"
    )
    parser.add_argument(
        "--language", "-l", help="Language context for normalization (e.g., en, es, de)"
    )
    parser.add_argument(
        "--dry-run",
        "-d",
    ...
    parser.add_argument(
        "--candidates-only",
        action="store_true",
        help="Only show merge candidates (same as --dry-run)",
    )
    parser.add_argument(
        "--neo4j-uri",
    ...
    parser.add_argument(
        "--neo4j-password", default=os.getenv("NEO4J_PASSWORD"), help="Neo4j password"
    )
    parser.add_argument(
        "--neo4j-database",
    ...
    if not args.neo4j_password:
        print(
            "❌ Neo4j password is required. Set NEO4J_PASSWORD environment variable or use --neo4j-password"
        )
        sys.exit(1)
    ...
    try:
        ...
        if args.dry_run:
            ...
            result = await cli.run_deduplication(
                args.collection, args.language, dry_run=False
            )
            print_results(result, dry_run=False)

            # Show metrics if available
            if hasattr(cli.deduplicator, "normalizer") and hasattr(
                cli.deduplicator.normalizer, "get_metrics"
            ):
                print("\n📊 Normalization Metrics:")
                metrics = cli.deduplicator.normalizer.get_metrics()
                print(f"   Entities processed: {metrics.get('entities_processed', 0)}")
                print(
                    f"   Entities normalized: {metrics.get('entities_normalized', 0)}"
                )
                print(
                    f"   Average confidence: {metrics.get('average_confidence', 0):.3f}"
                )
                print(
                    f"   Processing time: {metrics.get('total_processing_time', 0):.2f}s"
                )
