...
try:
    ...
    print(
        "⚠️  Warning: morag-stages not available. Stage functionality will be disabled."
    )
...
class MoRAGWebDAVProcessor:
    def __init__(
        ...
        if self.stages and STAGES_AVAILABLE:
            ...
            print(
                "⚠️  Warning: Stages requested but morag-stages not available. Stages will be skipped."
            )
        ...
        try:
            ...
            print(
                f"✓ Successfully initialized MoRAG Services with data output directory: {self.data_output_dir}"
            )
        except Exception as e:
    ...
    async def _execute_additional_stages(
        self, markdown_file_path: Path, original_file_path: str
    ) -> List[str]:
        ...
        try:
            ...
            for stage_type in stage_types:
                ...
                if result.success:
                    ...
                    print(
                        f"    ⚠️  Optional stage {stage_type.value} failed, continuing"
                    )
                    current_input_files = [markdown_file_path]
                else:
                    print(
                        f"    ✗ Stage {stage_type.value} failed: {result.error_message}"
                    )
                    break
    ...
    def get_all_files_recursive(self, folder_path: str) -> List[str]:
        ...
        try:
            ...
            if self.subdirectory:
                full_folder_path = (
                    f"{self.subdirectory.strip('/')}/{folder_path.strip('./')}"
                )
            else:
        ...
        try:
            ...
            def _collect_files(current_path: str):
                try:
                    ...
                    webdav_path = (
                        current_path if current_path not in [".", "./"] else "/"
                    )
                    ...
                    for item in items:
                        ...
                        if item.endswith("/") or "." not in item_name:
                            ...
                            try:
                                ...
                                print(
                                    f"📄 Adding as file (failed directory test): {item_path}"
                                )
    ...
    def should_process_file(
        self, file_path: str, folder_path: str, all_files: List[str]
    ) -> bool:
        """
        ...
        for existing_file in all_files:
            if existing_file == expected_markdown_path:
                print(
                    f"  ⏭️  Skipping {file_path} - markdown file already exists: {markdown_filename}"
                )
                return False
            elif existing_file == expected_intermediate_path:
                print(
                    f"  ⏭️  Skipping {file_path} - intermediate markdown file already exists: {intermediate_filename}"
                )
                return False
            elif existing_file == expected_data_path:
                print(
                    f"  ⏭️  Skipping {file_path} - data file already exists: {data_filename}"
                )
                return False
    ...
    async def process_file(
        self, remote_file_path: str, folder_path: str
    ) -> Optional[tuple]:
        """
        ...
        try:
            ...
            if file_extension in [
                ...
                from morag_video import VideoConfig, VideoService
                ...
                if "markdown" in output_files:
                    ...
                    print(
                        f"  ✓ Found generated markdown file: {markdown_file_path.name}"
                    )
                else:
                    print(
                        f"  ✗ No markdown file found in service output for {remote_file_path}"
                    )
                    return None
                ...
                from morag_audio import AudioConfig, AudioService
                ...
                from morag_document import DocumentConfig, DocumentService
                ...
                if not processing_result.success:
                    print(
                        f"  ✗ MoRAG processing failed for {remote_file_path}: {processing_result.error_message}"
                    )
                    return None
            ...
            try:
                ...
                print(
                    f"  ⚠️  Warning: Could not clean up {local_filename}: {cleanup_error}"
                )
            ...
            if is_cuda_oom_error(e):
                ...
                raise CUDAOutOfMemoryError(
                    f"CUDA out of memory while processing {remote_file_path}: {e}"
                )

            print(f"  ✗ Error processing {remote_file_path}: {e}")
            return None

    async def process_markdown_for_resume(
        self, remote_file_path: str, folder_path: str
    ) -> Optional[tuple]:
        """
        ...
        try:
            ...
            print(
                f"  ✓ Successfully processed markdown content for database population"
            )
    ...
    def upload_processed_files(
        ...
        for local_file in all_files:
            try:
                ...
                file_type = (
                    "data file" if local_file.endswith(".json") else "markdown file"
                )
                ...
                try:
                    ...
                    print(
                        f"  ⚠️  Warning: Could not clean up {filename}: {cleanup_error}"
                    )
    ...
    async def process_folder(self, folder_path: str):
        ...
        if self.resume_mode:
            print(
                f"📄 Resume mode: Processing existing markdown files for database population"
            )
        else:
        ...
        if not self.resume_mode:
            ...
            if content_type == "unknown":
                print(
                    f"✗ File extension '.{self.file_extension}' is not supported by MoRAG Services"
                )
                ...
                print(
                    "  Document: pdf, docx, xlsx, pptx, txt, md, html, csv, json, xml"
                )
                return
        ...
        if memory_info:
            print(
                f"🖥️  Initial GPU Memory: {memory_info['allocated_mb']} MB / {memory_info['total_mb']} MB ({memory_info['usage_percent']:.1f}% used)"
            )
        ...
        for i, file_path in enumerate(all_files, 1):
            ...
            if not self.should_process_file(file_path, folder_path, all_files):
                if self.resume_mode:
                    ...
                    print(
                        f"  ⏭️  Skipping {file_path} - wrong extension (looking for .{self.file_extension})"
                    )
                skipped_count += 1
                continue
            ...
            if memory_info and memory_info["usage_percent"] > 80:
                print(
                    f"  ⚠️  High GPU memory usage ({memory_info['usage_percent']:.1f}%) before processing {file_path}"
                )
            ...
            try:
                ...
                if processed_result:
                    ...
                    upload_success = self.upload_processed_files(
                        file_path, [markdown_file], data_files, folder_path
                    )
                    ...
                    if upload_success:
                        ...
                        print(
                            f"  ⚠️  Processing succeeded but upload failed for {file_path}"
                        )
                ...
                if memory_info:
                    ...
                    print(
                        f"     Allocated: {memory_info['allocated_mb']} MB ({memory_info['usage_percent']:.1f}%)"
                    )
        ...
        print(f"\n✅ Processing complete!")
        print(
            f"📊 Successfully processed and uploaded: {processed_count - upload_failed_count} files"
        )
        ...
        if upload_failed_count > 0:
            ...
            print(
                f"Check the markdown files in the current directory and upload them manually if needed."
            )
...
Examples:
  ...
  python webdav-processor.py -u https://webdav.example.com -U user -P pass -f /docs -e md --resume --stages chunker,fact-generator
    ...
    parser.add_argument("--username", "-U", required=True, help="WebDAV username")

    parser.add_argument("--password", "-P", required=True, help="WebDAV password")
    ...
    try:
        ...
        if content_type == "unknown":
            print(
                f"✗ File extension '.{args.extension}' is not supported by MoRAG Services"
            )
            print("Supported extensions:")
            print("  Video: mp4, avi, mov, mkv, wmv, flv, webm, m4v")
            print("  Audio: mp3, wav, m4a, flac, aac, ogg")
            ...
            print(
                f"✓ File extension '.{args.extension}' is supported (detected as {content_type})"
            )
    except Exception as e:
    ...
    processor = MoRAGWebDAVProcessor(
        args.url,
        args.subdirectory,
        args.username,
        args.password,
