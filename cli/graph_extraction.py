...
async def extract_facts_and_relationships(
    text: str, doc_id: str, domain: str = "general", context: Optional[str] = None
    ...
    return await extraction_service.extract_facts_and_relationships(
        text=text, doc_id=doc_id, domain=domain, context=context
    )
...
class FactExtractionService:
    ...
    def __init__(
        ...
        self.fact_extractor = FactExtractor(
            model_id=self.model, api_key=self.api_key, domain=self.domain
        )

        # Initialize fact graph builder
        self.fact_graph_builder = FactGraphBuilder(
            model_id=self.model, api_key=self.api_key
        )
...
class DatabaseIngestionService:
    ...
    async def ingest_to_neo4j(
        ...
        try:
            ...
            print(
                f"[OK] Stored {len(graph_entities)} entities and {len(graph_relations)} relations in Neo4j"
            )
            return result
...
async def extract_and_ingest(
    ...
    try:
        ...
        entities, relations = await extraction_service.extract_entities_and_relations(
            text=text_content, doc_id=doc_id, context=context
        )
            ...
            if ingestion_service.initialize_qdrant(qdrant_config):
                try:
                    point_ids = await ingestion_service.ingest_to_qdrant(
                        text_content=text_content, metadata=metadata or {}
                    )
                    ...
                    results["ingestion"]["qdrant"] = {"success": False, "error": str(e)}
            else:
                results["ingestion"]["qdrant"] = {
                    "success": False,
        ...
        if use_neo4j:
            if ingestion_service.initialize_neo4j(neo4j_config):
                try:
                    ...
                    results["ingestion"]["neo4j"] = {"success": True, **neo4j_result}
                except Exception as e:
                    results["ingestion"]["neo4j"] = {"success": False, "error": str(e)}
            else:
