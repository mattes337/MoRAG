...
async def execute_stage(args):
    ...
    if (
        hasattr(args, "fact_extraction_agent_model")
        and args.fact_extraction_agent_model
    ):
        agent_model_overrides["fact_extraction"] = args.fact_extraction_agent_model
    if (
        hasattr(args, "entity_extraction_agent_model")
        and args.entity_extraction_agent_model
    ):
        agent_model_overrides["entity_extraction"] = args.entity_extraction_agent_model
    if (
        hasattr(args, "relation_extraction_agent_model")
        and args.relation_extraction_agent_model
    ):
        agent_model_overrides[
            "relation_extraction"
        ] = args.relation_extraction_agent_model
    if (
        hasattr(args, "keyword_extraction_agent_model")
        and args.keyword_extraction_agent_model
    ):
        agent_model_overrides[
            "keyword_extraction"
        ] = args.keyword_extraction_agent_model
    ...
    if (
        hasattr(args, "content_analysis_agent_model")
        and args.content_analysis_agent_model
    ):
        agent_model_overrides["content_analysis"] = args.content_analysis_agent_model
    if (
        hasattr(args, "markdown_optimizer_agent_model")
        and args.markdown_optimizer_agent_model
    ):
        agent_model_overrides[
            "markdown_optimizer"
        ] = args.markdown_optimizer_agent_model
    ...
    try:
        result = await stage_manager.execute_stage(
            stage_type, [Path(args.input)], context
        )
...
async def execute_stage_chain(args):
    ...
    try:
        ...
        results = await stage_manager.execute_stage_chain(
            stage_types, [Path(args.input)], context
        )
        ...
        for result in results:
            if result.status == StageStatus.COMPLETED:
                ...
                print(
                    f"❌ Stage {result.stage_type.value} failed: {result.error_message}"
                )
                failed += 1
...
async def execute_full_pipeline(args):
    ...
    try:
        # Execute pipeline
        results = await stage_manager.execute_stage_chain(
            stages, [Path(args.input)], context
        )
...
def setup_parser():
    ...
    stage_parser.add_argument(
        "--llm-model", help="Default LLM model for all agents (e.g., gemini-1.5-flash)"
    )
    stage_parser.add_argument(
        "--fact-extraction-agent-model", help="LLM model for fact extraction agent"
    )
    stage_parser.add_argument(
        "--entity-extraction-agent-model", help="LLM model for entity extraction agent"
    )
    stage_parser.add_argument(
        "--relation-extraction-agent-model",
        help="LLM model for relation extraction agent",
    )
    stage_parser.add_argument(
        "--keyword-extraction-agent-model",
        help="LLM model for keyword extraction agent",
    )
    stage_parser.add_argument(
        "--summarization-agent-model", help="LLM model for summarization agent"
    )
    stage_parser.add_argument(
        "--content-analysis-agent-model", help="LLM model for content analysis agent"
    )
    stage_parser.add_argument(
        "--markdown-optimizer-agent-model",
        help="LLM model for markdown optimizer agent",
    )
    stage_parser.add_argument(
        "--chunking-agent-model", help="LLM model for chunking agent"
    )
    stage_parser.add_argument(
        "--llm-provider", help="Override LLM provider (e.g., gemini)"
    )
    stage_parser.add_argument(
        "--llm-temperature", type=float, help="Override LLM temperature"
    )
    stage_parser.add_argument(
        "--llm-max-tokens", type=int, help="Override LLM max tokens"
    )

    # Stage-specific overrides
    stage_parser.add_argument(
        "--chunk-size", type=int, help="Override chunk size for chunker"
    )
    stage_parser.add_argument(
        "--max-chunk-size",
        type=int,
        help="Override max chunk size for markdown optimizer",
    )
    ...
    stage_parser.add_argument(
        "--transcript-only",
        action="store_true",
        help="For YouTube URLs: only extract transcript, don't download video",
    )
    stage_parser.add_argument(
        "--transcript-language",
        help="For YouTube URLs: transcript language code (e.g., 'en', 'es')",
    )
    ...
    chain_parser.add_argument(
        "stages",
        help="Comma-separated stage names (e.g., 'markdown-conversion,chunker,fact-generator')",
    )
    ...
    chain_parser.add_argument(
        "--llm-provider", help="Override LLM provider for all stages"
    )
    chain_parser.add_argument(
        "--llm-temperature", type=float, help="Override LLM temperature for all stages"
    )
    chain_parser.add_argument(
        "--llm-max-tokens", type=int, help="Override LLM max tokens for all stages"
    )
    ...
    pipeline_parser.add_argument(
        "--optimize", action="store_true", help="Include markdown optimization stage"
    )
    pipeline_parser.add_argument(
        "--skip-stages", help="Comma-separated list of stages to skip"
    )
