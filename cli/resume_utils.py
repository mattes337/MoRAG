...
async def resume_from_process_result(
    process_result_data: Dict[str, Any],
    source_file: str,
    content_type: str,
    ...
    try:
        ...
        print_result(
            "Relations Extracted", str(result["graph_data"]["relations_count"])
        )
...
async def resume_from_ingestion_data(
    ...
    try:
        ...
        if "vector_data" in ingestion_data:
            ...
            for chunk_data in chunks_data:
                ...
                metadata = {
                    ...
                    "document_id": ingestion_data.get("source_info", {}).get(
                        "document_id", "unknown"
                    ),
        ...
        original_document_id = ingestion_data.get("document_id") or ingestion_data.get(
            "source_info", {}
        ).get("document_id", "unknown")
        ...
        for meta in chunk_metadata:
            ...
            if "chunk_id" in meta and original_document_id in meta["chunk_id"]:
                meta["chunk_id"] = meta["chunk_id"].replace(
        ...
        for i, chunk_data in enumerate(chunks_data):
            if (
                "chunk_id" in chunk_data
                and original_document_id in chunk_data["chunk_id"]
            ):
                chunk_data["chunk_id"] = chunk_data["chunk_id"].replace(
                    original_document_id, document_id
                )
        ...
        if "graph_data" in ingestion_data:
            ...
            if enhanced_processing and enhanced_processing.get("entities"):
                ...
                print(
                    f"🔍 DEBUG: Found {len(entities_data)} entities from enhanced processing"
                )
            else:
                # Fallback to basic entities data
                entities_data = graph_data_section.get("entities", [])
                print(
                    f"🔍 DEBUG: Found {len(entities_data)} entities from basic graph_data"
                )
            ...
            if not chunk_entity_mapping:
                print(
                    "🔧 DEBUG: chunk_entity_mapping missing, recreating from entities and chunks..."
                )
                chunk_entity_mapping = await _recreate_chunk_entity_mapping(
                    chunks,
                    entities_data,
                    ingestion_data.get("source_info", {}).get("chunk_size", 4000),
                )
                print(
                    f"🔧 DEBUG: Recreated chunk_entity_mapping with {len(chunk_entity_mapping)} chunks containing entities"
                )
            ...
            for entity_dict in entities_data:
                try:
                    ...
                    print(
                        f"[WARN]  Warning: Failed to create entity from {entity_dict.get('name', 'unknown')}: {e}"
                    )
                    continue
            ...
            for relation_dict in relations_data:
                try:
                    ...
                    print(
                        f"[WARN]  Warning: Failed to create relation {relation_dict.get('id', 'unknown')}: {e}"
                    )
                    continue
            ...
            graph_data = {
                ...
                "extraction_metadata": graph_data_section.get(
                    "extraction_metadata", {}
                ),
            }
        else:
        ...
        for db_name, result in results.items():
            if result.get("success", False):
                ...
                if "documents_written" in result:
                    print_result(
                        f"{db_name.upper()} Documents", str(result["documents_written"])
                    )
                if "entities_written" in result:
                    print_result(
                        f"{db_name.upper()} Entities", str(result["entities_written"])
                    )
                if "relations_written" in result:
                    print_result(
                        f"{db_name.upper()} Relations", str(result["relations_written"])
                    )
            else:
                print_result(
                    f"{db_name.upper()} Status",
                    f"[FAIL] Failed: {result.get('error', 'Unknown error')}",
                )
    ...
    for chunk_index, chunk_text in enumerate(chunks):
        ...
        for entity_dict in entities_data:
            ...
            if entity_name and entity_name in chunk_text_lower:
                ...
                print(
                    f"🔗 Found entity '{entity_dict.get('name', '')}' in chunk {chunk_index}"
                )

        if entities_in_chunk:
            chunk_entity_mapping[str(chunk_index)] = entities_in_chunk
            print(
                f"[INFO] Chunk {chunk_index} contains {len(entities_in_chunk)} entities"
            )

    print(
        f"[OK] Found entities in {len(chunk_entity_mapping)} out of {len(chunks)} chunks"
    )
...
def handle_resume_arguments(
    args, source_file: str, content_type: str, metadata: Optional[Dict[str, Any]] = None
):
    """
    ...
    if not hasattr(args, "use_process_result") or not args.use_process_result:
        if not hasattr(args, "use_ingestion_data") or not args.use_ingestion_data:
            ...
            if detected_files["ingestion_data"]:
                print(
                    f"[AUTO-DETECT] Found existing ingestion data: {detected_files['ingestion_data']}"
                )
                args.use_ingestion_data = detected_files["ingestion_data"]
            elif detected_files["process_result"]:
                print(
                    f"[AUTO-DETECT] Found existing process result: {detected_files['process_result']}"
                )
    ...
    if hasattr(args, "use_ingestion_data") and args.use_ingestion_data:
        ...
        try:
            ...
            print(
                "💡 Skipping processing and ingestion calculation, starting database writes..."
            )

            # Continue with database writes using the ingestion data
            success = asyncio.run(
                resume_from_ingestion_data(
                    ingestion_data,
                    source_file,
                    content_type,
                    use_qdrant=getattr(args, "qdrant", False),
