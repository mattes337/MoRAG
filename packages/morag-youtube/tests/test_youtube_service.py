...
@pytest.mark.asyncio
@patch.object(YouTubeProcessor, "process_url")
async def test_process_videos(
    mock_process_url, youtube_service, sample_download_result
):
...
async def test_process_playlist(
    mock_process_playlist, youtube_service, sample_download_result
):
...
@pytest.mark.asyncio
@patch.object(YouTubeProcessor, "process_url")
async def test_download_video_with_options(
    mock_process_url, youtube_service, sample_download_result, temp_dir
):
...
@pytest.mark.asyncio
@patch.object(YouTubeProcessor, "process_url")
async def test_download_subtitles(
    mock_process_url, youtube_service, sample_download_result, temp_dir
):
    ...
    subtitle_paths = await youtube_service.download_subtitles(
        url, languages=languages, output_dir=output_dir
    )
...
@pytest.mark.asyncio
@patch.object(YouTubeProcessor, "process_url")
async def test_download_thumbnail(
    mock_process_url, youtube_service, sample_download_result, temp_dir
):
