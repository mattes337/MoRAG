...
from .processor import YouTubeConfig, YouTubeProcessor
...
class ProcessYouTubeVideoTask(YouTubeProcessingTask):
    ...
    async def execute(
        ...
        logger.info("Starting YouTube video processing task", task_id=task_id, url=url)
        ...
        try:
            ...
            youtube_result = await self.youtube_processor.process_url(
                url, youtube_config
            )
            ...
            result = {
                ...
                "files": {
                    "video_path": str(youtube_result.video_path)
                    if youtube_result.video_path
                    else None,
                    "audio_path": str(youtube_result.audio_path)
                    if youtube_result.audio_path
                    else None,
            ...
            logger.info(
                "YouTube video processing task completed",
                task_id=task_id,
            ...
            logger.error(
                "YouTube video processing task failed", task_id=task_id, error=str(e)
            )
...
class ProcessYouTubePlaylistTask(YouTubeProcessingTask):
    ...
    async def execute(
        ...
        logger.info(
            "Starting YouTube playlist processing task", task_id=task_id, url=url
        )
        ...
        try:
            ...
            playlist_results = await self.youtube_processor.process_playlist(
                url, youtube_config
            )
            ...
            result = {
                ...
                "total_processing_time": sum(
                    r.processing_time for r in playlist_results
                ),
            ...
            logger.info(
                "YouTube playlist processing task completed",
                task_id=task_id,
            ...
            logger.error(
                "YouTube playlist processing task failed", task_id=task_id, error=str(e)
            )
...
class ExtractYouTubeMetadataTask(YouTubeProcessingTask):
    ...
    async def execute(self, url: str, task_id: Optional[str] = None) -> Dict[str, Any]:
        """Extract YouTube metadata without downloading."""

        logger.info(
            "Starting YouTube metadata extraction task", task_id=task_id, url=url
        )
        ...
        try:
            ...
            logger.info(
                "YouTube metadata extraction task completed",
                task_id=task_id,
                video_id=youtube_result.metadata.id,
                title=youtube_result.metadata.title,
            )
            ...
            logger.error(
                "YouTube metadata extraction task failed", task_id=task_id, error=str(e)
            )
...
async def extract_youtube_metadata(
    url: str, task_id: Optional[str] = None
) -> Dict[str, Any]:
    """Extract YouTube metadata without downloading."""
...
def create_celery_tasks(celery_app):
    ...
    def process_youtube_video_celery(
    ...
    @celery_app.task(bind=True)
    def process_youtube_playlist_celery(
        self, url: str, config: Optional[Dict[str, Any]] = None
    ):
