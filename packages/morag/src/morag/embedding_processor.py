...
class EmbeddingProcessor:
    ...
    async def generate_embeddings_and_metadata(
        ...
        try:
            ...
            metadata_list = await asyncio.gather(
                *[
                    self._generate_chunk_metadata_async(
                        chunk, i, content_type, base_metadata
                    )
            ...
            logger.info(
                "Generated embeddings and metadata with optimized batching",
            ...
            logger.error(
                "Failed to generate embeddings and metadata",
                error=str(e),
                chunk_count=len(chunks),
            )
            raise

    async def _generate_chunk_metadata_async(
        self,
        chunk_text: str,
        chunk_index: int,
        content_type: str,
        ...
        return self._add_chunk_metadata(
            chunk_text, chunk_index, content_type, base_metadata
        )
    ...
    def _add_audio_video_chunk_metadata(
        ...
        if timestamp_matches:
            ...
            start_seconds = (
                int(first_match[0]) * 3600
                + int(first_match[1]) * 60
                + int(first_match[2])
            )
            end_seconds = (
                int(last_match[0]) * 3600 + int(last_match[1]) * 60 + int(last_match[2])
            )
        ...
        speaker_matches = re.findall(
            r"(Speaker \w+|[A-Z][a-z]+ [A-Z][a-z]+):", chunk_text
        )
        if speaker_matches:
        ...
        if "duration" in chunk_metadata and chunk_metadata["duration"] > 0:
            speaking_rate = len(words) / (
                chunk_metadata["duration"] / 60
            )  # words per minute
        ...
        if any(
            ...
            for keyword in ["question", "answer", "q:", "a:"]
        ):
            chunk_metadata["contains_qa"] = True

        if any(
            keyword in chunk_text.lower() for keyword in ["music", "song", "melody"]
        ):
        ...
        chunk_metadata.update(
            {
                ...
                // 2,  # Divide by 2 since each block has opening and closing
    ...
    async def _generate_document_summary(self, content: str, content_type: str) -> str:
        ...
        try:
            if not self.fact_extractor:
                return (
                    f"Summary of {content_type} content ({len(content.split())} words)"
                )

            # Use fact extractor to generate summary
            summary_result = await self.fact_extractor.generate_summary(
