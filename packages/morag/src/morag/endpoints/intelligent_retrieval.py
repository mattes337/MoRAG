...
async def get_intelligent_retrieval_service(
    ...
    if request.neo4j_server:
        ...
        try:
            neo4j_storage = await DatabaseConnectionFactory.create_neo4j_storage(
                request.neo4j_server
            )
        except Exception as e:
            raise HTTPException(
                status_code=400, detail=f"Failed to create Neo4j connection: {str(e)}"
            )
    ...
    if request.qdrant_server:
        ...
        try:
            qdrant_storage = await DatabaseConnectionFactory.create_qdrant_storage(
                request.qdrant_server
            )
        except Exception as e:
            raise HTTPException(
                status_code=400, detail=f"Failed to create Qdrant connection: {str(e)}"
            )
    ...
    if not neo4j_storage:
        raise HTTPException(status_code=503, detail="Neo4j storage not available")

    if not qdrant_storage:
        raise HTTPException(status_code=503, detail="Qdrant storage not available")
...
async def intelligent_retrieval(request: IntelligentRetrievalRequest):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Intelligent retrieval failed: {str(e)}"
        )


@router.post("/intelligent-query/facts", response_model=RecursiveFactRetrievalResponse)
async def intelligent_fact_retrieval(request: RecursiveFactRetrievalRequest):
    ...
    try:
        ...
        response = await retry_on_overload(
            perform_retrieval, max_retries=8, base_delay=2.0, max_delay=120.0
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Intelligent fact retrieval failed: {str(e)}"
        )
