...
from morag.dependencies import (
    ...
    get_iterative_retriever,
    get_llm_client,
    get_reasoning_path_finder,
...
class MultiHopQuery(BaseModel):
    ...
    target_entities: Optional[List[str]] = Field(
        None, description="Optional target entities"
    )
    strategy: str = Field(
        "forward_chaining",
    ...
    max_iterations: int = Field(
        5, ge=1, le=10, description="Maximum refinement iterations"
    )
...
async def get_reasoning_status():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error checking reasoning status: {str(e)}"
        )
...
async def multi_hop_reasoning(
    ...
    if path_finder is None or iterative_retriever is None:
        raise HTTPException(
            status_code=503, detail="Reasoning components not properly initialized"
        )
    ...
    try:
        ...
        initial_context = RetrievalContext(
            ...
            entities={
                ...
                for entity in request.start_entities
            },
    ...
    if final_analysis:
        if final_analysis.is_sufficient:
            summary_parts.append(
                f"Context deemed sufficient with {final_analysis.confidence:.2f} confidence"
            )
        else:
            summary_parts.append(
                f"Context insufficient (confidence: {final_analysis.confidence:.2f})"
            )
...
async def fact_based_reasoning(request: RecursiveFactRetrievalRequest):
    ...
    try:
        ...
        if not service:
            raise HTTPException(
                status_code=503, detail="Recursive fact retrieval service not available"
            )
        ...
        logger.info(
            "Fact-based reasoning completed",
        ...
        logger.error(
            "Fact-based reasoning failed",
            error=str(e),
            error_type=type(e).__name__,
            query=request.user_query,
        )
        raise HTTPException(
            status_code=500, detail=f"Fact-based reasoning failed: {str(e)}"
        )
...
class FactBasedQuery(BaseModel):
    ...
    max_facts: int = Field(
        50, ge=1, le=500, description="Maximum total facts to collect"
    )
    facts_only: bool = Field(
        False, description="Return only facts without final answer synthesis"
    )
    language: str = Field(
        "en", description="Language for fact extraction and processing"
    )


@router.post("/query/facts/simple")
async def simple_fact_based_reasoning(request: FactBasedQuery):
    ...
    try:
        ...
        logger.info(
            "Simple fact-based reasoning completed",
            query_id=response.query_id,
            total_facts=len(response.final_facts),
        )
        ...
        logger.error(
            "Simple fact-based reasoning failed", error=str(e), query=request.query
        )
        raise HTTPException(
            status_code=500, detail=f"Simple fact-based reasoning failed: {str(e)}"
        )
