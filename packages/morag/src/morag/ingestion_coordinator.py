    ...
    async def ingest_content(
        ...
        try:
            ...
            chunks = self.chunk_processor.create_chunks(
                content, effective_chunk_size, effective_chunk_overlap, content_type, metadata
            )
            
            # Step 5: Generate embeddings and metadata using the chunk processor
            embeddings_data = await self.chunk_processor.generate_embeddings_and_metadata(
                content, chunks, source_path, content_type, metadata, document_id
            )
            
            # Step 6: Determine language for extraction if not provided
            effective_language = self._determine_language(language, processing_result, metadata)
            ...
            graph_data = await self._extract_graph_data(
                ...
                effective_chunk_size, effective_chunk_overlap, effective_language
            )
            
            # Step 8: Create complete ingest result using chunk processor
            ingest_result = self.chunk_processor.create_ingest_result(
                source_path, document_id, content, content_type, metadata,
                embeddings_data, graph_data, processing_result
            )
            
            # Step 9: Write ingest result file
            result_file_path = self.chunk_processor.write_ingest_result_file(source_path, ingest_result)
            
            # Step 10: Initialize databases using database handler
            await self.database_handler.initialize_databases(database_configs, embeddings_data)
            ...
            data_file_path = self.chunk_processor.write_ingest_data_file(source_path, ingest_data)
    ...
    async def _extract_graph_data(
        ...
        try:
            if not self.fact_extractor:
                logger.warning("Fact extractor not initialized, skipping graph data extraction")
                return None
            ...
            extraction_result = await self.fact_extractor.extract_facts_and_relationships(
                content=content,
                source_path=source_path,
            ...
            try:
                ...
                enhanced_processing = await enhanced_processor.process_facts_with_context(
            ...
            logger.info("Graph data extraction completed",
        ...
        try:
            ...
            if entities:
                ...
                for entity in entities:
                    ...
                    entity_text = f"{entity.get('name', '')} ({entity.get('type', 'Unknown')})"
                    if entity.get('description'):
                        entity_text += f": {entity['description']}"
                    entity_texts.append(entity_text)
                
                entity_embeddings = await self.embedding_service.generate_embeddings_batch(entity_texts)
            ...
            if facts:
                ...
                fact_embeddings = await self.embedding_service.generate_embeddings_batch(fact_texts)
            ...
            logger.info("Generated embeddings for graph data",
