...
async def download_remote_file(file_path: str, temp_dir: Path) -> Path:
    ...
    is_unc = file_path.startswith("\\\\") or (
        len(file_path) > 1 and file_path[1] == ":" and "\\" in file_path
    )

    if not is_url and not is_unc:
        raise HTTPException(
            status_code=400,
            detail=f"Invalid remote file path: {file_path}. Must be HTTP/HTTPS URL or UNC path.",
        )
    ...
    try:
                ...
                async with session.get(file_path) as response:
                    ...
                    if content_length:
                        try:
                            ...
                            logger.warning(
                                "Could not check file size limit", error=str(e)
                            )
        ...
        if isinstance(e, HTTPException):
            raise
        logger.error(
            "Failed to download remote file", remote_path=file_path, error=str(e)
        )
        raise HTTPException(
            status_code=500, detail=f"Failed to download remote file: {str(e)}"
        )
    ...
    if not hasattr(result, "content") or result.content is None:
        if hasattr(result, "document") and result.document:
            if hasattr(result.document, "raw_text") and result.document.raw_text:
                ...
                result.content = "\n\n".join(
                    [chunk.content for chunk in result.document.chunks]
                )
            else:
