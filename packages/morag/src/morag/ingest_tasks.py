...
from morag_services import GeminiEmbeddingService, QdrantVectorStorage
...
async def continue_ingestion_after_remote_processing(
    remote_job_id: str, content: str, metadata: Dict[str, Any], processing_time: float
) -> bool:
    ...
    try:
        ...
        if not remote_job:
            logger.error(
                "Remote job not found for continuation", remote_job_id=remote_job_id
            )
            return False
        ...
        if options.get("store_in_vector_db", True):
            ...
            logger.info(
                "Remote processing result stored via comprehensive ingestion",
                remote_job_id=remote_job_id,
                databases_used=len(ingestion_result.get("database_results", {})),
                qdrant_points=len(
                    ingestion_result.get("database_results", {})
                    .get("qdrant", {})
                    .get("point_ids", [])
                ),
                neo4j_entities=ingestion_result.get("database_results", {})
                .get("neo4j", {})
                .get("entities_stored", 0),
            )
        ...
        if webhook_url:
            send_webhook_notification(
                ...
                "SUCCESS",
                {
                    ...
                    "databases_used": list(
                        ingestion_result.get("database_results", {}).keys()
                    ),
        ...
        logger.error(
            "Failed to continue ingestion after remote processing",
            remote_job_id=remote_job_id,
            error=str(e),
        )
        return False
...
def send_webhook_notification(
    webhook_url: str, task_id: str, status: str, result: Optional[Dict[str, Any]] = None
):
...
def generate_document_id(source: str, content: Optional[str] = None) -> str:
    ...
    elif ("/" in source or "\\" in source or "." in source) and not source.startswith(
        ("http://", "https://")
    ):
    ...
    try:
        ...
        if use_content_checksum:
            ...
            if not replace_existing:
                existing_points = await vector_storage.search_by_metadata(
                    {"content_checksum": content_checksum}, limit=1
                )
                if existing_points:
                    logger.info(
                        "Document with same content checksum already exists, skipping",
        ...
        if document_id and replace_existing:
            ...
            point_ids = await vector_storage.replace_document(
                document_id, embeddings, chunk_metadata, collection_name
            )
        else:
            # Regular storage
            point_ids = await vector_storage.store_vectors(
                embeddings, chunk_metadata, collection_name
            )

        logger.info(
            "Content stored in vector database successfully",
            chunk_count=len(chunks),
            chunk_size=chunk_size,
        ...
        try:
            ...
            if options.get("remote", False):
                ...
                if content_type in ["audio", "video"]:
                    logger.info(
                        "Creating remote job for processing",
                        task_id=self.request.id,
                        content_type=content_type,
                    )
                    ...
                    logger.info(
                        "Remote job created, waiting for completion",
                        task_id=self.request.id,
                    ...
                    logger.info(
                        "Content type does not support remote processing, falling back to local",
                        content_type=content_type,
                    )
            ...
            if not file_exists:
                ...
                if parent_exists:
                    ...
                    try:
                        files_in_dir = (
                            list(parent_dir.glob("*")) if parent_dir.exists() else []
                        )
                        logger.error(
                            "File not found but parent directory exists",
                            file_path=file_path,
                        ...
                        logger.warning(
                            "Could not list parent directory contents",
                            parent_dir=str(parent_dir),
                            error=str(list_error),
                        )
                else:
                    logger.error(
                        "File and parent directory do not exist",
                        file_path=file_path,
                        parent_dir=str(parent_dir) if parent_dir else "None",
                    )

                raise FileNotFoundError(
                    f"Temporary file was cleaned up before processing: {file_path}"
                )

            self.update_state(
                state="PROGRESS", meta={"stage": "processing", "progress": 0.1}
            )
            ...
            self.update_state(
                state="PROGRESS", meta={"stage": "storing", "progress": 0.7}
            )
            ...
            if options.get("store_in_vector_db", True):
                ...
                if not document_id:
                    document_id = generate_document_id(
                        file_path, result.text_content or result.content
                    )
                ...
                result.metadata["stored_in_graph_db"] = bool(
                    ingestion_result.get("database_results", {}).get("neo4j")
                )

                # Maintain backward compatibility
                if ingestion_result.get("database_results", {}).get("qdrant"):
                    result.metadata["vector_point_ids"] = ingestion_result[
                        "database_results"
                    ]["qdrant"].get("point_ids", [])

            self.update_state(
                state="PROGRESS", meta={"stage": "completing", "progress": 0.9}
            )
            ...
            if webhook_url:
                send_webhook_notification(
                    ...
                    {
                        "chunks_processed": len(
                            result.metadata.get("vector_point_ids", [])
                        ),
            ...
            if isinstance(e, FileNotFoundError):
                logger.error(
                    "File ingestion task failed - temporary file was cleaned up prematurely",
                    file_path=file_path,
                    error=str(e),
    ...
    async def _ingest():
        ...
        try:
            ...
            if options.get("store_in_vector_db", True):
                ...
                if not document_id:
                    document_id = generate_document_id(
                        url, result.text_content or result.content
                    )
...
def ingest_batch_task(
    self, items: List[Dict[str, Any]], task_options: Optional[Dict[str, Any]] = None
):
    ...
    async def _ingest():
        ...
        try:
                ...
                try:
                    ...
                    if "url" in item:
                        ...
                        if not source_type:
                            ...
                            logger.info(
                                "Auto-detected content type for batch URL",
                                batch_index=i,
                                url=item["url"],
                        ...
                        source_type = item.get("source_type")
                        if not source_type:
                            source_type = api._detect_content_type_from_file(
                                Path(item["file_path"])
                            )
                            detected_source_type = source_type
                            logger.info(
                                "Auto-detected content type for batch file",
                                batch_index=i,
                    ...
                    if result.success and options.get("store_in_vector_db", True):
                        ...
                        ingestion_metadata = {
                            ...
                            or item.get("source_type", "unknown"),
                            "batch_index": i,
                    ...
                    logger.error(
                        "Failed to process batch item", item_index=i, error=str(e)
                    )
                    results.append(
                        {
                            "success": False,
            ...
            logger.error(
                "Batch ingestion task failed", item_count=len(items), error=str(e)
            )
