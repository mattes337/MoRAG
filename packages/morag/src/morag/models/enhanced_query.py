...
class EnhancedQueryRequest(BaseModel):
    ...
    use_fact_retrieval: bool = Field(default=True, description="Use fact-based retrieval instead of traditional hybrid retrieval")
    max_depth: int = Field(default=3, ge=1, le=10, description="Maximum traversal depth for fact extraction")
    ...
    facts_only: bool = Field(default=False, description="Return only facts without final answer synthesis")
    skip_fact_evaluation: bool = Field(default=False, description="Skip fact evaluation for faster processing")
    decay_rate: float = Field(default=0.2, ge=0.0, le=1.0, description="Rate at which fact scores decay per depth level")
    language: str = Field(default="en", description="Language for fact extraction and processing")
...
class FactInfo(BaseModel):
    ...
    source_qdrant_chunk_id: Optional[str] = Field(None, description="Source Qdrant chunk ID")
    source_metadata: Dict[str, Any] = Field(default_factory=dict, description="Source metadata")
    extracted_from_depth: int = Field(..., description="Depth at which this fact was extracted")
    score: float = Field(..., ge=0.0, le=1.0, description="Fact relevance score")
    final_decayed_score: float = Field(..., ge=0.0, le=1.0, description="Final score after decay")
...
class EnhancedQueryResponse(BaseModel):
    ...
    facts: Optional[List[FactInfo]] = Field(default=None, description="Extracted facts from graph traversal")
    final_answer: Optional[str] = Field(default=None, description="Final synthesized answer")
    initial_entities: Optional[List[str]] = Field(default=None, description="Initial entities identified")
    total_nodes_explored: Optional[int] = Field(default=None, description="Total nodes explored")
    max_depth_reached: Optional[int] = Field(default=None, description="Maximum depth reached")
    traversal_steps: Optional[List[TraversalStepInfo]] = Field(default=None, description="Traversal steps")
    total_raw_facts: Optional[int] = Field(default=None, description="Total raw facts extracted")
    total_scored_facts: Optional[int] = Field(default=None, description="Total scored facts")
    gta_llm_calls: Optional[int] = Field(default=None, description="GraphTraversalAgent LLM calls")
    fca_llm_calls: Optional[int] = Field(default=None, description="FactCriticAgent LLM calls")
    final_llm_calls: Optional[int] = Field(default=None, description="Final synthesis LLM calls")
