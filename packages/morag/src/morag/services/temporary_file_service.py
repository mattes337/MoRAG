...
class TemporaryFileService:
    ...
    async def create_session_dir(self, session_id: str) -> Path:
        ...
        logger.info("Created session directory", session_id=session_id, path=str(session_dir))
        return session_dir
    ...
    async def store_file(
        ...
        if new_size > max_size_bytes:
            raise ValueError(f"Session size limit exceeded: {new_size} > {max_size_bytes}")
        ...
        logger.info("Stored file", session_id=session_id, filename=filename, size=len(content))
        return metadata
    ...
    async def get_file(self, session_id: str, filename: str) -> Tuple[bytes, Dict[str, Any]]:
    ...
    async def list_session_files(self, session_id: str) -> Dict[str, Any]:
        ...
        if files:
            ...
            expires_at = (newest_time + timedelta(hours=self.retention_hours)).isoformat()
