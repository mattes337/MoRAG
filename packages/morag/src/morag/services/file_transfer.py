...
def get_file_transfer_service() -> Optional[FileTransferService]:
    """Get file transfer service if HTTP mode is enabled."""
    global _file_transfer_service
    
    # Check if HTTP file transfer mode is enabled
    server_url = os.getenv('MAIN_SERVER_URL')
    transfer_mode = os.getenv('FILE_TRANSFER_MODE', 'shared')
    
    if transfer_mode == 'http' and server_url:
        if _file_transfer_service is None:
            temp_dir = os.getenv('TEMP_DIR', '/tmp')
            _file_transfer_service = FileTransferService(server_url, temp_dir)
