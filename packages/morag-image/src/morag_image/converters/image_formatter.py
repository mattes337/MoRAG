...
class ImageFormatter:
    ...
    def _parse_content_sections(self, raw_content: str) -> Dict[str, str]:
        ...
        for line in lines:
            ...
            if any(
                ...
                for keyword in ["text content", "ocr", "extracted text"]
            ):
                ...
                keyword in line_lower
                for keyword in ["visual", "description", "image shows", "the image"]
            ):
                # Save previous section
                if current_content:
                    sections[current_section] = "\n".join(current_content).strip()
    ...
    def _format_visual_description(self, content: str) -> str:
        ...
        content = re.sub(
            r"^(The image shows?|This image|Image shows?)\s*:?\s*",
            "",
            content,
            flags=re.IGNORECASE,
        )
