...
from morag_core.interfaces.processor import BaseProcessor, ProcessingConfig
...
class ImageProcessor(BaseProcessor):
    ...
    async def process(
        self, file_path, config: Optional[ProcessingConfig] = None
    ...
    async def process_image(
        self, file_path: Path, config: ImageConfig
        ...
        try:
            ...
            if config.generate_caption:
                caption, confidence = await self._generate_caption(
                    processed_image_path, config
                )
            ...
            if config.extract_text:
                text, confidence = await self._extract_text(
                    processed_image_path, config
                )
    ...
    async def process_images(
        self, file_paths: List[Path], config: ImageConfig, max_concurrency: int = 3
        ...
        async def process_with_semaphore(
            file_path: Path,
        ) -> Tuple[Path, Optional[ImageProcessingResult], Optional[Exception]]:
        ...
        try:
            with Image.open(image_path) as img:
                ...
                if config.resize_max_dimension and (
                    img.width > config.resize_max_dimension
                    or img.height > config.resize_max_dimension
                ):
                    # Calculate new dimensions while maintaining aspect ratio
                    if img.width > img.height:
                        new_width = config.resize_max_dimension
                        new_height = int(
                            img.height * (config.resize_max_dimension / img.width)
                        )
                    else:
                        new_height = config.resize_max_dimension
                        new_width = int(
                            img.width * (config.resize_max_dimension / img.height)
                        )
            ...
            logger.error(
                "Image preprocessing failed", image_path=str(image_path), error=str(e)
            )
            raise ProcessingError(f"Image preprocessing failed: {str(e)}") from e

    async def _generate_caption(
        self, image_path: Path, config: ImageConfig
    ) -> Tuple[str, float]:
        ...
        try:
            ...
            logger.error(
                "Caption generation failed", image_path=str(image_path), error=str(e)
            )
            raise ExternalServiceError(
                f"Caption generation failed: {str(e)}", "gemini"
            ) from e
    ...
    async def _extract_text_tesseract(self, image_path: Path) -> Tuple[str, float]:
        ...
        try:
            ...
            logger.error(
                "Tesseract OCR failed", image_path=str(image_path), error=str(e)
            )
            raise ExternalServiceError(
                f"Tesseract OCR failed: {str(e)}", "tesseract"
            ) from e
    ...
    async def _extract_text_easyocr(self, image_path: Path) -> Tuple[str, float]:
        ...
        try:
            ...
            logger.error("EasyOCR failed", image_path=str(image_path), error=str(e))
