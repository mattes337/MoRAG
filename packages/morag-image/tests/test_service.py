...
from morag_image.processor import ImageConfig, ImageMetadata, ImageProcessingResult
...
async def test_process_image(image_service, test_image_path, mock_processing_result):
    ...
    with patch.object(
        image_service.processor, "process_image", return_value=mock_processing_result
    ):
...
@pytest.mark.asyncio
async def test_process_image_with_config(
    image_service, test_image_path, mock_processing_result
):
...
async def test_error_handling(image_service, test_image_path):
    ...
    with patch.object(
        image_service.processor, "process_image", side_effect=Exception("Test error")
    ):
