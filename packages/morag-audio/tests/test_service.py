...
import pytest
from morag_audio.converters import AudioConversionResult, AudioConverter
...
"""
    ...
    mock.convert_to_markdown.return_value = AudioConversionResult(
        content=markdown_content, metadata={}, processing_time=0.5, success=True
    )
...
async def test_service_initialization(audio_config):
    ...
    with patch("morag_audio.service.AudioProcessor", return_value=MagicMock()), patch(
        "morag_audio.service.AudioConverter", return_value=MagicMock()
    ):
...
async def test_process_file_success(mock_processor, mock_converter):
    ...
    with patch(
        "morag_audio.service.AudioProcessor", return_value=mock_processor
    ), patch("morag_audio.service.AudioConverter", return_value=mock_converter), patch(
...
async def test_process_file_with_output(mock_processor, mock_converter, tmp_path):
    ...
    with patch(
        ...
        with patch.object(
            service, "_save_output_files", new_callable=AsyncMock
        ) as mock_save:
...
async def test_process_file_error_handling(audio_config):
    ...
    with patch(
        "morag_audio.service.AudioProcessor", return_value=mock_processor
    ), patch("morag_audio.service.AudioConverter", return_value=MagicMock()), patch(
        "pathlib.Path.exists", return_value=True
    ):
        service = AudioService(config=audio_config)
