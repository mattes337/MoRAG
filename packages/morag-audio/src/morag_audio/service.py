...
class AudioService:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                not hasattr(self.processor, "transcriber")
                or not self.processor.transcriber
            ):
                return {
                    "status": "unhealthy",
        ...
        try:
            ...
            if self.embedding_service and result.success:
                try:
                    ...
                    logger.info(
                        "Generated embeddings for transcript and segments",
            ...
            if result.success:
                ...
                if output_format == "markdown":
                    markdown_result = await self.converter.convert_to_markdown(
                        result, conversion_options
                    )
                    formatted_content = markdown_result.content
                elif output_format == "json":
                    formatted_content = await self.converter.convert_to_json(
                        result, conversion_options
                    )
    ...
    async def _save_output_files(
        ...
        metadata_dict = {
            ...
            "has_speaker_diarization": any(
                getattr(seg, "speaker", None) for seg in result.segments
            )
            if result.segments
            else False,
            "has_topic_segmentation": any(
                getattr(seg, "topic_id", None) is not None for seg in result.segments
            )
            if result.segments
            else False,
            "metadata": result.metadata,
