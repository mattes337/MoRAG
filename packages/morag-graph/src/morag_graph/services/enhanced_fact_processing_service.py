...
class EnhancedFactProcessingService:
    ...
    def __init__(
        ...
        entity_normalizer: Optional[LLMEntityNormalizer] = None,
        llm_service=None,
    ):
        ...
        self.categorization_service = EntityCategorizationService(
    ...
    async def process_facts_with_entities(
        ...
        try:
            ...
            existing_facts, new_facts = await self._identify_existing_and_new_facts(
                facts
            )
            stored_facts = []
            ...
            if new_facts:
                ...
                self.logger.info(
                    f"Stored {len(stored_facts)} new facts, {len(existing_facts)} facts already existed"
                )
            else:
                self.logger.info(
                    f"All {len(existing_facts)} facts already exist in database"
                )
    ...
    async def _create_entities_from_facts(self, facts: List[Fact]) -> List[Entity]:
            ...
            try:
                entity = Entity(
                    ...
                    attributes={
                        ...
                        "normalization_confidence": entity_data.get(
        ...
        if entities_for_categorization:
            try:
                domain = entities_for_categorization[0].attributes.get(
                    "domain", "general"
                )
                categorized_entities = (
                    await self.categorization_service.categorize_entities(
                ...
                self.logger.warning(
                    f"Entity categorization failed, storing without categories: {e}"
                )
        ...
        self.logger.info(
            f"Created {len(created_entities)} normalized entities from fact subjects/objects"
        )
    ...
    async def _process_entity_from_fact_component(
        ...
        if self.entity_normalizer:
            try:
                normalization_result = await self.entity_normalizer.normalize_entity(
                    entity_name
                )
                ...
                self.logger.warning(
                    f"Entity normalization failed for '{entity_name}': {e}"
                )
    ...
    async def _create_keyword_entities(self, facts: List[Fact]) -> List[Entity]:
        ...
        for fact in facts:
            ...
            for keyword in fact.keywords:
                ...
                if len(keyword_name) < 3 or keyword_name.lower() in [
                    "the",
                    "and",
                    "for",
                    "with",
                ...
                if self.entity_normalizer:
                    try:
                        normalization_result = (
                            await self.entity_normalizer.normalize_entity(keyword_name)
                        )
                        normalized_name = normalization_result.normalized
                        normalization_confidence = normalization_result.confidence
                    except Exception as e:
                        self.logger.warning(
                            f"Keyword normalization failed for '{keyword_name}': {e}"
                        )
            ...
            try:
                entity = Entity(
                    ...
                    attributes={
                        ...
                        "normalization_confidence": keyword_data[
        ...
        self.logger.info(
            f"Created {len(created_keyword_entities)} normalized keyword entities"
        )
        return created_keyword_entities

    async def _create_fact_entity_relations(
        self, facts: List[Fact], entities: List[Entity]
    ) -> List[Relation]:
            ...
            try:
                ...
                for entity_name in fact.structured_metadata.primary_entities:
                    if entity_name and entity_name.strip():
                        ...
                        entity_normalized = (
                            await self._normalize_entity_name_for_lookup(
                                entity_name.strip()
                            )
                        )
                        if entity_normalized in entity_lookup:
                            entity = entity_lookup[entity_normalized]
                            relation_type = await self._determine_entity_relation_type(
                                fact, entity_name
                            )
                            ...
                            if success:
                                created_relations.append(
                                    f"fact_{fact.id}_to_{entity.id}_{relation_type}"
                                )
                ...
                for concept_name in fact.structured_metadata.domain_concepts:
                    if concept_name and concept_name.strip():
                        ...
                        if concept_normalized in entity_lookup:
                            ...
                            if success:
                                created_relations.append(
                                    f"fact_{fact.id}_to_{concept_entity.id}_{relation_type}"
                                )
                    ...
                    for keyword in fact.keywords:
                        if keyword and keyword.strip():
                            ...
                            if keyword_normalized in entity_lookup:
                                ...
                                relation_type = (
                                    await self._determine_keyword_relation_type(
                                        fact, keyword
                                    )
                                )
                                success = await self._create_fact_entity_relation(
                                ...
                                if success:
                                    created_relations.append(
                                        f"fact_{fact.id}_to_{keyword_entity.id}_{relation_type}"
                                    )

            except Exception as e:
                self.logger.warning(
                    f"Failed to create relations for fact {fact.id}: {e}"
                )
                continue
    ...
    async def _normalize_entity_name_for_lookup(self, entity_name: str) -> str:
        ...
        if self.entity_normalizer:
            try:
                ...
                self.logger.warning(
                    f"Entity normalization failed for lookup '{entity_name}': {e}"
                )
                return entity_name.strip()
        else:
            return entity_name.strip()

    async def _determine_entity_relation_type(
        self, fact: Fact, entity_name: str
    ) -> str:
...
Return only the relationship type in uppercase, no explanation."""
        ...
        try:
            if hasattr(self, "llm_service") and self.llm_service:
                ...
                if (
                    relation_type
                    and len(relation_type) < 50
                    and relation_type.replace("_", "").isalpha()
                ):
        ...
        if any(
            word in fact_text_lower
            for word in ["causes", "leads to", "results in", "triggers", "creates"]
        ):
            return "CAUSES"
        elif any(
            word in fact_text_lower
            for word in ["treats", "heals", "cures", "helps", "fixes"]
        ):
            return "TREATS"
        elif any(
            word in fact_text_lower
            for word in ["prevents", "blocks", "stops", "avoids"]
        ):
            return "PREVENTS"
        elif any(
            word in fact_text_lower
            for word in ["improves", "enhances", "boosts", "increases"]
        ):
            return "IMPROVES"
        elif any(
            word in fact_text_lower
            for word in ["contains", "includes", "has", "comprises"]
        ):
            return "CONTAINS"
        else:
...
Return only the relationship type in uppercase, no explanation."""
    ...
    def _fallback_keyword_relation_type(self, fact: Fact, keyword: str) -> str:
        ...
        if any(
            word in fact_text
            for word in ["method", "technique", "approach", "way", "process"]
        ):
            return "USES_METHOD"
        elif any(
            word in fact_text for word in ["tool", "instrument", "device", "equipment"]
        ):
            return "USES_TOOL"
        elif any(
            word in fact_text for word in ["location", "place", "where", "in", "at"]
        ):
            return "OCCURS_IN"
    ...
    async def _identify_existing_and_new_facts(
        ...
        self.logger.info(
            f"Fact analysis: {len(existing_facts)} existing, {len(new_facts)} new"
        )
    ...
    async def _create_fact_chunk_relations(self, facts: List[Fact]) -> int:
        ...
        for fact in facts:
            ...
            try:
                ...
                self.logger.debug(
                    f"Processed chunk-fact relation: {fact.source_chunk_id} -> {fact.id}"
                )

            except Exception as e:
                self.logger.warning(
                    f"Failed to create chunk-fact relation for fact {fact.id}: {e}"
                )
                continue

        self.logger.info(
            f"Processed {processed_relations} fact-chunk relations (created or updated)"
        )
    ...
    async def _generate_and_store_embeddings(
        ...
        try:
            ...
            if not api_key:
                self.logger.warning(
                    "GEMINI_API_KEY not found - skipping embedding generation"
                )
            ...
            entity_embedding_service = EntityEmbeddingService(
                self.neo4j_storage, gemini_service
            )
            ...
            for entity in entities:
                try:
                    embedding = (
                        await entity_embedding_service.generate_entity_embedding(
                            entity.id
                        )
                    )
                    if embedding:
                        success = await entity_embedding_service.store_entity_embedding(
                            entity.id, embedding
                        )
                        if success:
                            entities_embedded += 1
                            self.logger.debug(
                                f"Stored embedding for entity {entity.id}"
                            )
                except Exception as e:
                    self.logger.warning(
                        f"Failed to generate/store embedding for entity {entity.id}: {e}"
                    )
                    continue
            ...
            for fact in facts:
                try:
                    embedding = await fact_embedding_service.generate_fact_embedding(
                        fact.id
                    )
                    if embedding:
                        success = await fact_embedding_service.store_fact_embedding(
                            fact.id, embedding
                        )
                        if success:
                            facts_embedded += 1
                    ...
                    self.logger.warning(
                        f"Failed to generate/store embedding for fact {fact.id}: {e}"
                    )
                    continue

            self.logger.info(
                f"Generated and stored embeddings: {entities_embedded} entities, {facts_embedded} facts"
            )
            return {
    ...
    async def _create_fact_entity_relation(
        self, fact: Fact, entity: Entity, relation_type: str
    ) -> bool:
        ...
        try:
            ...
            result = await self.neo4j_storage._connection_ops._execute_query(
                query,
                {
                    ...
                    "entity_id": entity.id,
                    "confidence": min(
                        fact.extraction_confidence or 0.8, entity.confidence or 0.8
                    ),
            ...
            if result and len(result) > 0:
                self.logger.debug(
                    f"Created {relation_type} relation between fact {fact.id} and entity {entity.id}"
                )
                return True
            else:
                self.logger.warning(
                    f"Failed to create relation between fact {fact.id} and entity {entity.id}"
                )
                return False
    ...
    async def _create_chunk_entity_relations(
        self, facts: List[Fact], entities: List[Entity]
    ) -> int:
        ...
        for chunk_id, chunk_fact_list in chunk_facts.items():
            try:
                    ...
                    if fact.keywords:
                        for keyword in fact.keywords:
                            if keyword and keyword.strip():
                                ...
                                if keyword_normalized in entity_lookup:
                                    chunk_entities.add(
                                        entity_lookup[keyword_normalized]
                                    )
                ...
                for entity in chunk_entities:
                    ...
                    result = await self.neo4j_storage._connection_ops._execute_query(
                        query, {"chunk_id": chunk_id, "entity_id": entity.id}
                    )

                    if result and len(result) > 0:
                        created_relations += 1
                        self.logger.debug(
                            f"Created MENTIONS relation between chunk {chunk_id} and entity {entity.id}"
                        )

            except Exception as e:
                self.logger.warning(
                    f"Failed to create chunk-entity relations for chunk {chunk_id}: {e}"
                )
                continue
