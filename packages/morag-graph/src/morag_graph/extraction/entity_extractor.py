...
class EntityExtractor:
    ...
    def _create_entity_prompt(self) -> str:
        ...
        if self.entity_types:
            type_descriptions = "\n".join(
                [f"- {name}: {desc}" for name, desc in self.entity_types.items()]
            )
    ...
    async def extract(
        ...
        try:
            ...
            result = await never_fail_extraction(
                main_extraction, fallback_strategies, operation_name="entity extraction"
            )
    ...
    def _convert_to_entities(
        self, result: Any, source_doc_id: Optional[str]
    ) -> List[Entity]:
        """Convert LangExtract results to MoRAG Entity objects."""
        entities = []
    ...
    async def _convert_to_entities_async(
        self, result: Any, source_doc_id: Optional[str]
    ) -> List[Entity]:
        ...
        if entity_names and self.normalizer:
            try:
                ...
                self.logger.warning(
                    "Batch normalization failed, using original names", error=str(e)
                )
        ...
        for i, data in enumerate(entity_data):
            try:
                ...
                if self._is_generic_entity_name(entity_name):
                    self.logger.debug(
                        f"Skipping entity with generic name: '{entity_name}'"
                    )
                    continue
    ...
    def _create_basic_entities_from_text(
            ...
            for entity_type, type_patterns in patterns.items():
                for pattern in type_patterns:
                    ...
                    for match in matches:
                        ...
                        if len(entity_name) > 2 and not self._is_generic_entity_name(
                            entity_name
                        ):
                            entity = Entity(
                                name=entity_name,
    ...
    def _create_minimal_entities_from_text(
        self, text: str, source_doc_id: Optional[str]
    ) -> List[Entity]:
        ...
        try:
            ...
            self.logger.warning(
                "Minimal entity extraction failed", error=str(e), text_length=len(text)
            )
            return []
