...
class QueryOptimizer:
    ...
    async def execute_optimized_query(
        self, query_name: str, params: Dict[str, Any], **template_params
    ) -> List[Dict[str, Any]]:
    ...
    async def _execute_with_stats(
        self, query: str, params: Dict[str, Any], query_name: str = "custom"
    ) -> List[Dict[str, Any]]:
    ...
    async def find_entities_by_fact_count(
        self, min_facts: int = 10, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """Find entities with at least min_facts attached facts."""
        return await self.execute_optimized_query(
            "find_entities_by_fact_count", {"min_facts": min_facts, "limit": limit}
        )

    async def find_cooccurring_entities(
        self, entity_id: str, total_facts: int, min_share: float = 0.18, limit: int = 50
    ) -> List[Dict[str, Any]]:
    ...
    async def find_duplicate_relationships(
        ...
        return await self.execute_optimized_query(
            "find_duplicate_relationships", {"limit": limit}
        )

    async def find_orphaned_relationships(
        self, limit: int = 1000
    ) -> List[Dict[str, Any]]:
        """Find relationships pointing to non-existent entities."""
        return await self.execute_optimized_query(
            "find_orphaned_relationships", {"limit": limit}
        )

    async def get_relationship_type_summary(self) -> List[Dict[str, Any]]:
        """Get summary of all relationship types and their counts."""
        return await self.execute_optimized_query("get_relationship_type_summary", {})
    ...
    async def get_fact_relationship_types(self) -> List[Dict[str, Any]]:
        ...
        """
        return await self.execute_optimized_query("get_fact_relationship_types", {})
    ...
    async def batch_delete_relationships(
        ...
        return await self.execute_optimized_query(
            "batch_delete_relationships", {"relationship_ids": relationship_ids}
        )
    ...
    def get_query_performance_summary(self) -> Dict[str, Any]:
        ...
        avg_execution_time = (
            sum(s.execution_time for s in successful_queries) / len(successful_queries)
        ...
        return {
            ...
            "success_rate": len(successful_queries) / len(self.query_stats)
            if self.query_stats
            else 0,
        }
