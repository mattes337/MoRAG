...
from ..models import Document, DocumentChunk, Entity, Fact, Graph, Relation
...
class Neo4jStorage(BaseStorage):
    ...
    async def create_document_contains_chunk_relation(
        ...
        if not self._document_ops:
            raise RuntimeError("Connection not initialized")
        return await self._document_ops.create_document_contains_chunk_relation(
    ...
    async def store_entity_with_chunk_references(
        ...
        if not self._entity_ops:
            raise RuntimeError("Connection not initialized")
        return await self._entity_ops.store_entity_with_chunk_references(
            entity, chunk_ids
        )
    ...
    async def update_entity_chunk_references(
        ...
        if not self._entity_ops:
            raise RuntimeError("Connection not initialized")
        return await self._entity_ops.update_entity_chunk_references(
            entity_id, chunk_ids
        )
    ...
    async def create_chunk_mentions_entity_relation(
        ...
        if not self._entity_ops:
            raise RuntimeError("Connection not initialized")
        return await self._entity_ops.create_chunk_mentions_entity_relation(
    ...
    async def create_chunk_contains_fact_relation(
        ...
        if not self._fact_ops:
            raise RuntimeError("Connection not initialized")
        return await self._fact_ops.create_chunk_contains_fact_relation(
    ...
    async def get_entity_relations(
        ...
        if not self._relation_ops:
            raise RuntimeError("Connection not initialized")
        return await self._relation_ops.get_entity_relations(
    ...
    async def find_path(
        ...
        if not self._query_ops:
            raise RuntimeError("Connection not initialized")
        return await self._query_ops.find_path(
    ...
    async def search_entities_by_content(
        ...
        if not self._query_ops:
            raise RuntimeError("Connection not initialized")
        return await self._query_ops.search_entities_by_content(
            search_term, entity_type, limit
        )
    ...
    async def find_related_entities(
        ...
        if not self._query_ops:
            raise RuntimeError("Connection not initialized")
        return await self._query_ops.find_related_entities(
            entity_id, relation_types, max_depth, limit
        )
    ...
    async def find_shortest_paths(
        ...
        if not self._query_ops:
            raise RuntimeError("Connection not initialized")
        return await self._query_ops.find_shortest_paths(
            source_entity_id, target_entity_id, max_depth
        )
