...
class EntityBatch(BaseModel):
    ...
    embeddings: Dict[str, List[float]] = Field(
        default_factory=dict, description="Entity embeddings by entity ID"
    )
...
class FactBatch(BaseModel):
    ...
    fact_relations: List[FactRelation] = Field(
        default_factory=list, description="List of fact relationships"
    )
    embeddings: Dict[str, List[float]] = Field(
        default_factory=dict, description="Fact embeddings by fact ID"
    )
    chunk_mappings: Dict[str, List[str]] = Field(
        default_factory=dict, description="Chunk ID to fact IDs mapping"
    )
...
class DocumentBatch(BaseModel):
    ...
    chunk_embeddings: Dict[str, List[float]] = Field(
        default_factory=dict, description="Chunk embeddings by chunk ID"
    )
...
class AtomicStorageBatch(BaseModel):
    ...
    # Data batches
    documents: DocumentBatch = Field(
        default_factory=DocumentBatch, description="Document and chunk batch"
    )
    entities: EntityBatch = Field(
        default_factory=EntityBatch, description="Entity batch"
    )
    facts: FactBatch = Field(default_factory=FactBatch, description="Fact batch")
    relations: RelationBatch = Field(
        default_factory=RelationBatch, description="Relation batch"
    )
    ...
    def validate_consistency(self) -> List[str]:
        ...
        for fact in self.facts.facts:
            if fact.source_chunk_id and fact.source_chunk_id not in chunk_ids:
                errors.append(
                    f"Fact {fact.id} references non-existent chunk {fact.source_chunk_id}"
                )
        ...
        for relation in self.relations.relations:
            if (
                ...
                and relation.source_entity_id not in fact_ids
            ):
                errors.append(
                    f"Relation {relation.id} references non-existent source {relation.source_entity_id}"
                )
            if (
                relation.target_entity_id not in entity_ids
                and relation.target_entity_id not in fact_ids
            ):
                errors.append(
                    f"Relation {relation.id} references non-existent target {relation.target_entity_id}"
                )
        ...
        for fact_relation in self.facts.fact_relations:
            if fact_relation.source_fact_id not in fact_ids:
                errors.append(
                    f"FactRelation {fact_relation.id} references non-existent source fact {fact_relation.source_fact_id}"
                )
            if fact_relation.target_fact_id not in fact_ids:
                errors.append(
                    f"FactRelation {fact_relation.id} references non-existent target fact {fact_relation.target_fact_id}"
                )
...
class AtomicStorageResult(BaseModel):
    ...
    stored_counts: Dict[str, int] = Field(
        default_factory=dict, description="Count of items stored by type"
    )
    errors: List[str] = Field(
        default_factory=list, description="List of errors encountered"
    )
    storage_duration_seconds: float = Field(
        default=0.0, description="Time taken for storage operation"
    )
    database_results: Dict[str, Any] = Field(
        default_factory=dict, description="Database-specific results"
    )
