...
from .types import EntityId, RelationAttributes, RelationId
...
class Relation(BaseModel):
    ...
    def to_neo4j_relationship(self) -> Dict[str, Any]:
        ...
        properties.pop(
            "context", None
        )  # Full chunk text - entities already refer to chunks
        properties.pop(
            "source_doc_id", None
        )  # Document reference - entities have chunk references
    ...
    def from_neo4j_relationship(
        cls, relationship: Any, source_entity_id: EntityId, target_entity_id: EntityId
    ) -> "Relation":
        ...
        if "id" not in relationship_dict:
            relationship_dict[
                "id"
            ] = f"{source_entity_id}_{target_entity_id}_{hash(str(relationship))}"
        ...
        if "attributes" in relationship_dict and isinstance(
            relationship_dict["attributes"], str
        ):
            try:
                relationship_dict["attributes"] = json.loads(
