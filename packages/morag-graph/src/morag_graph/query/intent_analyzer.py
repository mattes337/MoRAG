...
class QueryIntentAnalyzer:
        ...
        for intent, patterns in self.intent_patterns.items():
            ...
            if entities:
                ...
                if intent == "factual" and "PERSON" in entity_types:
                    ...
                    score += 0.1
                elif intent == "causal" and any(
                    t in entity_types for t in ["EVENT", "CONCEPT"]
                ):
                    score += 0.1

            # Question word adjustments
            if intent == "factual" and any(
                word in query_lower for word in ["what", "who", "where", "when"]
            ):
                score += 0.1
            elif intent == "explanatory" and any(
                word in query_lower for word in ["how", "why"]
            ):
                score += 0.1
            ...
            if matched_patterns:
                self.logger.debug(
                    f"Intent '{intent}' matched patterns: {matched_patterns}, score: {score:.3f}"
                )
    ...
    def is_complex_query(self, query: str, entities: List[QueryEntity]) -> bool:
        ...
        if any(
            word in query.lower() for word in ["compare", "difference", "versus", "vs"]
        ):
            return True
