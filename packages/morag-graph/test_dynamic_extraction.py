...
async def test_dynamic_extraction():
    ...
    llm_config = LLMConfig(
        provider="gemini", api_key="test_key", model="gemini-1.5-flash"
    )
    ...
    print(
        f"  System prompt includes 'semantic meaning': {'semantic meaning' in entity_prompt}"
    )
    print(
        f"  System prompt includes 'not limit yourself': {'not limit yourself' in entity_prompt}"
    )
    ...
    print(
        f"  Entity types (constraints): {len(static_entity_extractor.entity_types)} types"
    )
    ...
    print(
        f"  Relation types (examples): {len(relation_extractor.relation_types)} types"
    )

    relation_prompt = relation_extractor.get_system_prompt()
    print(
        f"  System prompt includes 'semantic meaning': {'semantic meaning' in relation_prompt}"
    )
    print(
        f"  System prompt includes 'not limit yourself': {'not limit yourself' in relation_prompt}"
    )
    ...
    print(
        f"  Relation types (constraints): {len(static_relation_extractor.relation_types)} types"
    )
    ...
    empty_relation_extractor = RelationExtractor(
        llm_config, dynamic_types=False, relation_types={}
    )

    empty_relation_prompt = empty_relation_extractor.get_system_prompt()
    print(
        f"  System prompt handles empty types: {'no specific relation types' in empty_relation_prompt}"
    )
