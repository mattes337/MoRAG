...
def llm_config():
    ...
    return LLMConfig(
        provider="gemini", model="gemini-1.5-flash", api_key="test-key", temperature=0.1
    )
...
class TestIntentionBasedExtraction:
    ...
    async def test_entity_extraction_with_intention(self, entity_extractor):
        ...
        with patch.object(
            entity_extractor, "call_llm", new_callable=AsyncMock
        ) as mock_llm:
            ...
            assert (
                "Document intention: Heal the pineal gland for spiritual enlightenment"
                in user_message
            )
    ...
    async def test_relation_extraction_with_intention(self, relation_extractor):
        ...
        with patch.object(
            relation_extractor, "call_llm", new_callable=AsyncMock
        ) as mock_llm:
            mock_llm.return_value = mock_response
            ...
            relations = await relation_extractor.extract(
                text, entities=entities, intention=intention
            )
    ...
    @pytest.mark.asyncio
    async def test_organizational_intention_extraction(
        self, entity_extractor, relation_extractor
    ):
        ...
        with patch.object(
            ...
            assert any(
                e.name == "TechCorp" and e.type == "ORGANIZATION" for e in entities
            )
    ...
    async def test_extract_entities_alias_method(self, entity_extractor):
        ...
        with patch.object(
            ...
            entities = await entity_extractor.extract_entities(
                "Test text", source_doc_id="test-doc", intention="Test intention"
            )
