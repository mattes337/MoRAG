...
pytestmark = pytest.mark.skip(
    reason="EntityDeduplicator interface has changed - tests need to be updated"
)
...
def sample_entities():
    ...
    return [
        ...
        Entity(name="AI", type="TECHNOLOGY", confidence=0.8, source_doc_id="doc1"),
        Entity(name="A.I.", type="TECHNOLOGY", confidence=0.7, source_doc_id="doc2"),
        Entity(
        ...
        Entity(name="ML", type="TECHNOLOGY", confidence=0.8, source_doc_id="doc2"),
        Entity(name="John Smith", type="PERSON", confidence=0.9, source_doc_id="doc1"),
        Entity(name="Smith, John", type="PERSON", confidence=0.8, source_doc_id="doc2"),
        Entity(
            name="Google Inc.",
            type="ORGANIZATION",
            confidence=0.9,
        ...
        Entity(
            name="Google", type="ORGANIZATION", confidence=0.8, source_doc_id="doc2"
        ),
    ]
...
class TestEntityDeduplicationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_deduplication_pipeline_without_llm(
        self, neo4j_storage, sample_entities
    ):
    ...
    async def test_large_entity_set_performance(self, neo4j_storage):
        ...
        # Initialize deduplicator
        deduplicator = EntityDeduplicator(
            neo4j_storage, llm_service=None, config={"dry_run": True, "batch_size": 10}
        )
...
class TestEntityDeduplicationWithMockLLM:
    ...
    async def test_deduplication_with_mock_llm(self, neo4j_storage, sample_entities):
        ...
        deduplicator = EntityDeduplicator(
            neo4j_storage, llm_service=mock_llm_service, config={"dry_run": True}
        )
...
class TestEntityDeduplicationConfiguration:
    ...
    @pytest.mark.asyncio
    async def test_different_confidence_thresholds(
        self, neo4j_storage, sample_entities
    ):
        ...
        assert (
            result_low["merge_candidates_found"]
            >= result_high["merge_candidates_found"]
        )
    ...
    async def test_batch_size_configuration(self, neo4j_storage):
        ...
        entities = [
            Entity(
                name=f"entity_{i}", type="CUSTOM", confidence=0.8, source_doc_id="doc1"
            )
            for i in range(25)
        ]
        ...
        deduplicator = EntityDeduplicator(
            neo4j_storage, llm_service=None, config={"dry_run": True, "batch_size": 5}
        )
