...
def sample_entities():
    return [
        Entity(name="John Doe", type="PERSON", source_doc_id="doc_test_abc123"),
        Entity(name="Acme Corp", type="ORGANIZATION", source_doc_id="doc_test_abc123"),
    ]
...
class TestGraphBuilderChecksum:
    ...
    @pytest.mark.asyncio
    async def test_process_document_new_document(
        self, mock_storage, mock_llm_config, sample_entities, sample_relations
    ):
        """Test processing a new document."""
        with patch(
            "morag_graph.builders.graph_builder.EntityExtractor"
        ) as mock_entity_extractor, patch(
            "morag_graph.builders.graph_builder.RelationExtractor"
        ) as mock_relation_extractor:
            # Setup mock extractors
            mock_entity_extractor.return_value.extract = AsyncMock(
                return_value=sample_entities
            )
            mock_relation_extractor.return_value.extract = AsyncMock(
                return_value=sample_relations
            )

            builder = GraphBuilder(storage=mock_storage, llm_config=mock_llm_config)
            ...
            with patch.object(builder, "_store_entities_and_relations") as mock_store:
                ...
                result = await builder.process_document(
                    content="Test content", document_id="test_doc"
                )
                ...
                assert (
                    result.cleanup_result.entities_deleted == 0
                )  # New document, nothing to clean

                # Verify checksum was stored
                assert "test_doc" in mock_storage.checksums

    @pytest.mark.asyncio
    async def test_process_document_unchanged_document(
        self, mock_storage, mock_llm_config
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_document_changed_document(
        self, mock_storage, mock_llm_config, sample_entities, sample_relations
    ):
        ...
        with patch(
            ...
            old_checksum = builder.checksum_manager.calculate_document_checksum(
                old_content
            )
            ...
            old_entity = Entity(
                name="Old Entity", type="PERSON", source_doc_id=document_id
            )
            ...
            with patch.object(builder, "_store_entities_and_relations") as mock_store:
                ...
                result = await builder.process_document(
                    content=new_content, document_id=document_id
                )
                ...
                new_checksum = builder.checksum_manager.calculate_document_checksum(
                    new_content
                )
                assert mock_storage.checksums[document_id] == new_checksum

    @pytest.mark.asyncio
    async def test_process_document_with_metadata_change(
        self, mock_storage, mock_llm_config, sample_entities, sample_relations
    ):
        ...
        with patch(
            ...
            with patch.object(builder, "_store_entities_and_relations") as mock_store:
                ...
                result = await builder.process_document(
                    content=content, document_id=document_id, metadata=new_metadata
                )

                assert isinstance(result, GraphBuildResult)
                assert (
                    result.skipped is False
                )  # Should reprocess due to metadata change
    ...
    def test_graph_build_result_with_cleanup(self):
        ...
        cleanup_result = CleanupResult(
            document_id="test_doc", entities_deleted=2, relations_deleted=1
        )
