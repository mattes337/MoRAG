...
class Settings(BaseSettings):
    ...
    gemini_generation_model: str = Field(
        default="gemini-2.0-flash", alias="MORAG_GEMINI_GENERATION_MODEL"
    )
    gemini_embedding_model: str = Field(
        default="text-embedding-004", alias="MORAG_GEMINI_EMBEDDING_MODEL"
    )
    gemini_vision_model: str = Field(
        default="gemini-1.5-flash", alias="MORAG_GEMINI_VISION_MODEL"
    )

    # Embedding Configuration
    embedding_batch_size: int = Field(default=100, alias="MORAG_EMBEDDING_BATCH_SIZE")
    enable_batch_embedding: bool = Field(
        default=True, alias="MORAG_ENABLE_BATCH_EMBEDDING"
    )
    embedding_delay_between_batches: float = Field(
        default=0.05, alias="MORAG_EMBEDDING_DELAY_BETWEEN_BATCHES"
    )
    rate_limit_per_minute: int = Field(default=200, alias="MORAG_RATE_LIMIT_PER_MINUTE")
    enable_performance_monitoring: bool = Field(
        default=True, alias="MORAG_ENABLE_PERFORMANCE_MONITORING"
    )

    # LLM Batch Configuration
    llm_batch_size: int = Field(
        default=10,
        alias="MORAG_LLM_BATCH_SIZE",
        ge=1,
        le=50,
        description="Number of prompts to batch together for LLM calls",
    )
    enable_llm_batching: bool = Field(
        default=True,
        alias="MORAG_ENABLE_LLM_BATCHING",
        description="Enable batching of LLM requests to reduce API calls",
    )
    llm_batch_delay: float = Field(
        default=1.0,
        alias="MORAG_LLM_BATCH_DELAY",
        ge=0.0,
        le=10.0,
        description="Delay between LLM batch requests in seconds",
    )
    llm_max_batch_tokens: int = Field(
        default=800000,
        alias="MORAG_LLM_MAX_BATCH_TOKENS",
        ge=100000,
        le=1000000,
        description="Maximum tokens per batch request (considering Gemini's 1M context limit)",
    )
    llm_batch_timeout: int = Field(
        default=120,
        alias="MORAG_LLM_BATCH_TIMEOUT",
        ge=30,
        le=300,
        description="Timeout for batch LLM requests in seconds",
    )
    ...
    slow_query_threshold: float = Field(
        default=5.0, alias="MORAG_SLOW_QUERY_THRESHOLD"
    )  # seconds
    cpu_threshold: float = Field(
        default=80.0, alias="MORAG_CPU_THRESHOLD"
    )  # percentage
    memory_threshold: float = Field(
        default=80.0, alias="MORAG_MEMORY_THRESHOLD"
    )  # percentage
    metrics_enabled: bool = Field(default=True, alias="MORAG_METRICS_ENABLED")

    # Retry Configuration
    retry_indefinitely: bool = Field(
        default=True, alias="MORAG_RETRY_INDEFINITELY"
    )  # Enable indefinite retries for rate limits
    retry_base_delay: float = Field(
        default=1.0, alias="MORAG_RETRY_BASE_DELAY"
    )  # Base delay in seconds
    retry_max_delay: float = Field(
        default=300.0, alias="MORAG_RETRY_MAX_DELAY"
    )  # Maximum delay in seconds (5 minutes)
    retry_exponential_base: float = Field(
        default=2.0, alias="MORAG_RETRY_EXPONENTIAL_BASE"
    )  # Exponential backoff multiplier
    retry_jitter: bool = Field(
        default=True, alias="MORAG_RETRY_JITTER"
    )  # Add random jitter to delays

    # Entity Extraction Retry Configuration
    entity_extraction_max_retries: int = Field(
        default=20, alias="MORAG_ENTITY_EXTRACTION_MAX_RETRIES"
    )  # Max retries for entity extraction
    entity_extraction_retry_base_delay: float = Field(
        default=1.0, alias="MORAG_ENTITY_EXTRACTION_RETRY_BASE_DELAY"
    )  # Base delay for entity extraction retries
    entity_extraction_retry_max_delay: float = Field(
        default=300.0, alias="MORAG_ENTITY_EXTRACTION_RETRY_MAX_DELAY"
    )  # Max delay for entity extraction retries

    # Celery Task Configuration
    celery_task_soft_time_limit: int = Field(
        default=120 * 60, alias="MORAG_CELERY_TASK_SOFT_TIME_LIMIT"
    )  # 2 hours (7200 seconds) - soft limit
    celery_task_time_limit: int = Field(
        default=150 * 60, alias="MORAG_CELERY_TASK_TIME_LIMIT"
    )  # 2.5 hours (9000 seconds) - hard limit
    celery_worker_prefetch_multiplier: int = Field(
        default=1, alias="MORAG_CELERY_WORKER_PREFETCH_MULTIPLIER"
    )  # Tasks per worker process
    celery_worker_max_tasks_per_child: int = Field(
        default=1000, alias="MORAG_CELERY_WORKER_MAX_TASKS_PER_CHILD"
    ...
    max_upload_size_bytes: int = Field(
        default=5 * 1024 * 1024 * 1024, alias="MORAG_MAX_UPLOAD_SIZE_BYTES"
    )  # 5GB default

    # File Size Limits (in bytes) - for specific content types
    max_document_size: int = Field(
        default=100 * 1024 * 1024, alias="MORAG_MAX_DOCUMENT_SIZE"
    )  # 100MB
    max_audio_size: int = Field(
        default=2 * 1024 * 1024 * 1024, alias="MORAG_MAX_AUDIO_SIZE"
    )  # 2GB
    max_video_size: int = Field(
        default=5 * 1024 * 1024 * 1024, alias="MORAG_MAX_VIDEO_SIZE"
    )  # 5GB
    max_image_size: int = Field(
        default=50 * 1024 * 1024, alias="MORAG_MAX_IMAGE_SIZE"
    )  # 50MB
    ...
    log_rotation: str = Field(
        default="daily", alias="MORAG_LOG_ROTATION"
    )  # daily, weekly, size

    # Environment settings
    environment: str = Field(
        default="development", alias="MORAG_ENVIRONMENT"
    ...
    def validate_collection_name(cls, v):
        ...
        if not v:
            logger.warning(
                "QDRANT_COLLECTION_NAME not set, using default 'morag_documents'"
            )
...
def log_configuration_debug(settings: Settings) -> None:
    ...
    logger.info(
        ...
        MORAG_ENABLE_PAGE_BASED_CHUNKING=os.getenv(
            "MORAG_ENABLE_PAGE_BASED_CHUNKING", "NOT SET"
        ),
        MORAG_DEFAULT_CHUNKING_STRATEGY=os.getenv(
            "MORAG_DEFAULT_CHUNKING_STRATEGY", "NOT SET"
        ),
    )
