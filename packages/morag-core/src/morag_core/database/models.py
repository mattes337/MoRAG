...
class User(Base):
    ...
    database_servers = relationship(
        "DatabaseServer", back_populates="user", cascade="all, delete-orphan"
    )
...
class DatabaseServer(Base):
    ...
    databases = relationship(
        "Database", back_populates="server", cascade="all, delete-orphan"
    )
...
class Database(Base):
    ...
    server_id = Column(
        String(36), ForeignKey("database_servers.id", ondelete="CASCADE"), nullable=False
    )
    ...
    # Constraints
    __table_args__ = (UniqueConstraint("name", "user_id", name="unique_database_name_per_user"),)
...
class Document(Base):
    ...
    database_id = Column(
        String(36), ForeignKey("databases.id", ondelete="SET NULL"), nullable=True
    )
...
def create_database_server(
    session, user_id: str, name: str, db_type: DatabaseType, host: str, port: int, **kwargs
) -> DatabaseServer:
...
def create_database(session, user_id: str, server_id: str, name: str, description: str) -> Database:
    ...
    database = Database(
        user_id=user_id, server_id=server_id, name=name, description=description
    )
...
def create_document(
    session, user_id: str, name: str, doc_type: str, database_id: str = None
) -> Document:
    """Create a new document."""
    document = Document(
        user_id=user_id, database_id=database_id, name=name, type=doc_type
    )
...
def create_api_key(session, user_id: str, name: str, key: str) -> ApiKey:
    ...
    api_key = ApiKey(user_id=user_id, name=name, key=key)
    session.add(api_key)
    session.commit()
...
def create_job(
    session, user_id: str, document_id: str, document_name: str, document_type: str
) -> Job:
...
def update_document_count(session, database_id: str):
    ...
    if database:
        count = (
            ...
            .filter(
                Document.database_id == database_id, Document.state != DocumentState.DELETED
            )
            .count()
        )
        database.document_count = count
        database.last_updated = datetime.now(timezone.utc)
        session.commit()


def update_job_progress(
    session, job_id: str, status: JobStatus, percentage: int, summary: str = ""
):
