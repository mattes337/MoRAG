...
from morag_video.processor import (
    VideoConfig,
...
class VideoService:
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    if result.audio_processing_result.segments:
                        ...
                        segment_texts = [
                            seg.text for seg in result.audio_processing_result.segments
                        ]
                        segment_embeddings = await self.embedding_service.embed_batch(
                            segment_texts
                        )
            ...
            if output_format == "markdown":
                try:
                    ...
                    formatted_content = await self.converter.convert_to_markdown(
                        result, conversion_options
                    )
                except Exception as e:
            ...
            if result.audio_processing_result:
                response["audio"] = {
                    "transcript_length": len(result.audio_processing_result.transcript)
                    if result.audio_processing_result.transcript
                    else 0,
                    "language": result.audio_processing_result.metadata.get(
                        "language", "unknown"
                    ),
                    "segments_count": len(result.audio_processing_result.segments)
                    if result.audio_processing_result.segments
                    else 0,
                    "has_speaker_diarization": result.audio_processing_result.metadata.get(
                        "has_speaker_info", False
                    ),
                    "has_topic_segmentation": result.audio_processing_result.metadata.get(
                        "has_topic_info", False
                    ),
                    "has_embeddings": hasattr(
                        result.audio_processing_result, "embeddings"
                    ),
                }
            ...
            if result.ocr_results:
                response["ocr"] = {
                    ...
                    "has_text": any(
                        text.strip() for text in result.ocr_results.values()
                    ),
                }
    ...
    async def convert_result_to_markdown(
        ...
        try:
            ...
            if isinstance(content, VideoProcessingResult):
                ...
                markdown_content = await self.converter.convert_to_markdown(
                    content, options
                )
            elif isinstance(content, dict):
                # Create markdown directly from dictionary content
                logger.info("Converting dictionary content to markdown")
                markdown_content = await self._convert_dict_to_markdown(
                    content, json_result.get("metadata", {})
                )
                ...
                logger.warning(
                    f"Unknown content type: {type(content)}, returning original"
                )
                return json_result
    ...
    async def _convert_dict_to_markdown(
    ...
    async def _save_output_files(
        ...
        if result.audio_processing_result and result.audio_processing_result.transcript:
            ...
            try:
                transcript_path.write_text(
                    result.audio_processing_result.transcript, encoding="utf-8"
                )
                output_files["transcript"] = str(transcript_path)
                logger.debug("Saved transcript", path=str(transcript_path))
            except Exception as e:
                logger.error(
                    "Failed to save transcript", path=str(transcript_path), error=str(e)
                )
        ...
        if markdown_content:
            ...
            try:
                ...
                logger.error(
                    "Failed to save markdown", path=str(markdown_path), error=str(e)
                )

        # Save segments as JSON if available
        if (
            result.audio_processing_result
            and hasattr(result.audio_processing_result, "segments")
            ...
            try:
                ...
                logger.error(
                    "Failed to save segments", path=str(segments_path), error=str(e)
                )
        ...
        try:
            ...
            logger.error(
                "Failed to save metadata", path=str(metadata_path), error=str(e)
            )
        ...
        if result.ocr_results:
            ...
            try:
                ocr_path.write_text(
                    json.dumps(result.ocr_results, indent=2), encoding="utf-8"
                )
                output_files["ocr"] = str(ocr_path)
                logger.debug("Saved OCR results", path=str(ocr_path))
            except Exception as e:
                logger.error(
                    "Failed to save OCR results", path=str(ocr_path), error=str(e)
                )
    ...
    async def _convert_to_json(self, result: VideoProcessingResult) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                ...
                topics_metadata = result.audio_processing_result.metadata.get(
                    "topics", []
                )
                ...
                if topics_metadata:
                    ...
                    for topic_info in topics_metadata:
                        topic_data = {
                            ...
                            "title": topic_info.get(
                                "title", f"Topic {topic_info.get('id', '')}"
                            ),
                        ...
                        topic_id = (
                            topic_info.get("id", "").replace("TOPIC_", "")
                            if isinstance(topic_info.get("id"), str)
                            else topic_info.get("id")
                        )
                        ...
                        for segment in result.audio_processing_result.segments:
                            if (
                                hasattr(segment, "topic_id")
                                and segment.topic_id == topic_id
                            ):
                                sentence = {
                    ...
                    topic_data = {
                        "timestamp": int(
                            result.audio_processing_result.segments[0].start
                        )
                        if result.audio_processing_result.segments
                        else 0,
    ...
    def _create_comprehensive_metadata(
        self, file_path: Path, result: VideoProcessingResult
    ) -> Dict[str, Any]:
        """Create comprehensive metadata including all required document fields."""
