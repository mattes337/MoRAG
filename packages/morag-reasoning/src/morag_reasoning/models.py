...
class MultiHopQuery(BaseModel):
    ...
    strategy: ReasoningStrategy = Field(
        ReasoningStrategy.FORWARD_CHAINING, description="Reasoning strategy"
    )
...
class PathScore(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=10.0, description="Confidence in score (0-10)"
    )
...
class ContextGap(BaseModel):
    ...
    entities_needed: List[str] = Field(
        default_factory=list, description="Entities needed to fill gap"
    )
    relations_needed: List[str] = Field(
        default_factory=list, description="Relations needed to fill gap"
    )
    priority: float = Field(
        1.0, ge=0.0, le=1.0, description="Priority for filling this gap"
    )
...
class ContextAnalysis(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in sufficiency assessment"
    )
    gaps: List[ContextGap] = Field(
        default_factory=list, description="Identified context gaps"
    )
    reasoning: str = Field(..., description="Reasoning for the assessment")
    suggested_queries: List[str] = Field(
        default_factory=list, description="Suggested queries to fill gaps"
    )
...
class ReasoningResult(BaseModel):
    ...
    final_confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Final confidence in result"
    )
...
class ReasoningConfig(BaseModel):
    ...
    sufficiency_threshold: float = Field(
        0.8, ge=0.0, le=1.0, description="Confidence threshold for stopping"
    )
    max_paths_to_select: int = Field(
        10, ge=1, le=50, description="Maximum paths to select"
    )
    llm_temperature: float = Field(
        0.1, ge=0.0, le=2.0, description="LLM temperature for reasoning"
    )
...
class ReasoningContext:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "paths": [
                path.to_dict() if hasattr(path, "to_dict") else str(path)
