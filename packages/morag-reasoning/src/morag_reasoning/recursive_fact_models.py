...
class RawFact(BaseModel):
    ...
    source_property: Optional[str] = Field(
        None, description="Optional property name if fact from property"
    )
    ...
    extracted_from_depth: int = Field(
        ...,
        description="Integer representing the traversal depth when this fact was extracted (0 for initial entities)",
    )
...
class ScoredFact(BaseModel):
    ...
    score: float = Field(
        ..., ge=0.0, le=1.0, description="Relevance score assigned by FCA"
    )
...
class FinalFact(BaseModel):
    ...
    score: float = Field(
        ..., ge=0.0, le=1.0, description="Original relevance score assigned by FCA"
    )
    source_description: str = Field(
        ..., description="A user-friendly description of the source"
    )
    final_decayed_score: float = Field(
        ..., ge=0.0, le=1.0, description="Score after applying depth-based decay"
    )
...
class RecursiveFactRetrievalRequest(BaseModel):
    ...
    neo4j_database: Optional[str] = Field(
        None, description="Neo4j database name (optional)"
    )
    qdrant_collection: Optional[str] = Field(
        None, description="Qdrant collection name (optional)"
    )
    language: Optional[str] = Field(
        None, description="Language for processing (optional)"
    )
...
class RecursiveFactRetrievalResponse(BaseModel):
    ...
    traversal_steps: List[TraversalStep] = Field(
    ...
    final_facts: List[FinalFact] = Field(
        ..., description="Final facts after decay and filtering"
    )

    # LLM usage
    gta_llm_calls: int = Field(
...
class GTAResponse(BaseModel):
    ...
    extracted_facts: List[RawFact] = Field(
        ..., description="Raw facts extracted from the current node"
    )
    next_nodes_to_explore: str = Field(
        ..., description="Decision about next nodes to explore"
    )
...
class FCAResponse(BaseModel):
    ...
    scored_fact: ScoredFact = Field(
        ..., description="The fact with assigned score and source description"
    )
