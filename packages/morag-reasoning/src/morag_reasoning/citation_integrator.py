...
from .citation_manager import CitedFact, CitationFormat, SourceReference
...
class CitationIntegrator:
    ...
    async def integrate_citations(
        ...
        """
        if not facts:
            return self._create_uncited_response(response, "No facts provided for citation")
        ...
        try:
            ...
            citation_matches = await self._find_citation_matches(response, facts, options)
            
            if not citation_matches:
                return self._create_uncited_response(response, "No citation matches found")
            ...
            verification_status = "verified" if options.verify_citations else "not_verified"
