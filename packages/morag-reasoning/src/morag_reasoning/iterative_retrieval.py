...
class IterativeRetriever:
    ...
    async def refine_context(
        self, query: str, initial_context: RetrievalContext
    ) -> RetrievalContext:
        ...
        while iteration_count < self.max_iterations:
            ...
            if (
                ...
                and analysis.confidence >= self.sufficiency_threshold
            ):
...
"""
        ...
        for entity_id, entity_data in list(context.entities.items())[
            :10
...
Format as JSON:
{
  ...
  "suggested_queries": ["What is the relationship between X and Y?"]
    ...
    async def _retrieve_additional(
        self, query: str, gaps: List[ContextGap], current_context: RetrievalContext
    ) -> RetrievalContext:
        ...
        for gap in sorted_gaps[:3]:  # Process top 3 gaps to avoid overwhelming
            try:
                if gap.gap_type == "missing_entity":
                    ...
                    for entity_name in gap.entities_needed:
                        if hasattr(self.graph_engine, "get_entity_details"):
                            entity_info = await self.graph_engine.get_entity_details(
                                entity_name
                            )
                            if entity_info:
                                additional_context.entities[entity_name] = entity_info
                        elif hasattr(self.graph_engine, "get_entity"):
                            entity_info = await self.graph_engine.get_entity(
                                entity_name
                            )
                            if entity_info:
                    ...
                    for relation_name in gap.relations_needed:
                        if hasattr(self.graph_engine, "get_relations_by_type"):
                            relations = await self.graph_engine.get_relations_by_type(
                                relation_name
                            )
                ...
                self.logger.error(
                    f"Error retrieving additional info for gap {gap.gap_type}: {str(e)}"
                )
                continue
    ...
    def _merge_context(
        self, current_context: RetrievalContext, additional_context: RetrievalContext
    ) -> RetrievalContext:
