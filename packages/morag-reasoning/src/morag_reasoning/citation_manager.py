...
class CitationManager:
    ...
    async def _process_citation_batch(
        self, scored_facts: List[ScoredFact], citation_format: CitationFormat
    ) -> List[CitedFact]:
        ...
        for scored_fact in scored_facts:
            try:
                cited_fact = await self._create_cited_fact(scored_fact, citation_format)
    ...
    async def _create_cited_fact(
        self, scored_fact: ScoredFact, citation_format: CitationFormat
    ) -> CitedFact:
        ...
        if not sources and fact.source_entities:
            for entity_id in fact.source_entities[: self.max_sources_per_fact]:
                try:
                    ...
                    logger.warning(
                        f"Failed to create source reference for entity {entity_id}: {e}"
                    )
    ...
    async def _create_source_reference_from_document(
        self, document_id: str, fact: ExtractedFact
    ...
    async def _create_source_reference_from_entity(
        self, entity_id: str, fact: ExtractedFact
    ) -> Optional[SourceReference]:
    ...
    def _generate_citation_text(
        self, sources: List[SourceReference], citation_format: CitationFormat
    ) -> str:
    ...
    def _extract_chunk_id_from_fact(
        self, fact: ExtractedFact, document_id: str
    ) -> Optional[str]:
    ...
    def _extract_page_number_from_fact(
        self, fact: ExtractedFact, cached_ref: Dict[str, Any]
    ) -> Optional[int]:
    ...
    def _extract_timestamp_from_fact(
        self, fact: ExtractedFact, cached_ref: Dict[str, Any]
    ) -> Optional[str]:
