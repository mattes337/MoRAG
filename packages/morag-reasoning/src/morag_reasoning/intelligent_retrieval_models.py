...
class KeyFact(BaseModel):
    ...
    relevance_to_query: float = Field(
        ge=0.0, le=1.0, description="Relevance to user query"
    )
    fact_type: str = Field(
        ...,
    ...
    supporting_entities: List[str] = Field(
        default_factory=list, description="Entities that support this fact"
    )
...
class IntelligentRetrievalRequest(BaseModel):
    ...
    max_iterations: int = Field(
        default=5, ge=1, le=10, description="Maximum recursive iterations"
    )
    max_entities_per_iteration: int = Field(
        default=10, ge=1, le=50, description="Max entities to explore per iteration"
    )
    max_paths_per_entity: int = Field(
        default=5, ge=1, le=20, description="Max paths to consider per entity"
    )
    ...
    include_debug_info: bool = Field(
        default=False, description="Include debug information in response"
    )
    ...
    neo4j_server: Optional[DatabaseServerConfig] = Field(
        None, description="Custom Neo4j server configuration"
    )
    qdrant_server: Optional[DatabaseServerConfig] = Field(
        None, description="Custom Qdrant server configuration"
    )

    # Retry configuration for handling model overload (503 errors)
    max_retries: int = Field(
        default=8, ge=1, le=20, description="Maximum retry attempts for overload errors"
    )
...
class IntelligentRetrievalResponse(BaseModel):
    ...
    completeness_score: float = Field(
        ge=0.0, le=1.0, description="Estimated completeness of information"
    )
    ...
    debug_info: Optional[Dict[str, Any]] = Field(
        None, description="Debug information if requested"
    )
...
class FactExtractionRequest(BaseModel):
    ...
    max_facts: int = Field(
        default=20, ge=1, le=100, description="Maximum facts to extract"
    )
...
class PathFollowingRequest(BaseModel):
    ...
    available_paths: List[EntityPath] = Field(
        ..., description="Available paths to consider"
    )
