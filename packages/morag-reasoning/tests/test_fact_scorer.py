...
class TestFactRelevanceScorer:
    ...
    async def test_basic_fact_scoring(self, mock_llm_client, sample_facts):
        ...
        assert all(
            isinstance(sf.scoring_dimensions, ScoringDimensions) for sf in scored_facts
        )
    ...
    async def test_relevance_focused_scoring(self, mock_llm_client, sample_facts):
        ...
        einstein_fact = next(
            (
                ...
                if "Einstein" in sf.fact.content and "relativity" in sf.fact.content
            ),
            None,
        )
    ...
    async def test_scoring_dimensions(self, mock_llm_client, sample_facts):
        ...
        config = {"llm_enabled": False, "semantic_enabled": False}
    ...
    async def test_confidence_scoring(self, mock_llm_client, sample_facts):
        ...
        if high_confidence_facts and low_confidence_facts:
            avg_high_score = sum(
                sf.overall_score for sf in high_confidence_facts
            ) / len(high_confidence_facts)
            avg_low_score = sum(sf.overall_score for sf in low_confidence_facts) / len(
    ...
    async def test_fact_type_scoring(self, mock_llm_client, sample_facts):
        ...
        direct_facts = [
            sf for sf in scored_facts if sf.fact.fact_type == FactType.DIRECT
        ]
    ...
    async def test_keyword_relevance_calculation(self, mock_llm_client):
        ...
        # Test partial match
        relevance = scorer._calculate_keyword_relevance(
            "Einstein worked at Princeton", "Einstein"
        )
        assert relevance > 0.0

        # Test no match
        relevance = scorer._calculate_keyword_relevance(
            "Completely unrelated content", "Einstein"
        )
        assert relevance == 0.0
