...
from .models import StageContext, StageResult, StageStatus, StageType
...
class WebhookNotifier:
    ...
    async def notify_stage_completion(
        self, webhook_url: str, stage_result: StageResult, context: StageContext
    ) -> bool:
    ...
    def _create_webhook_payload(
        self, stage_result: StageResult, context: StageContext
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "stage": {
                ...
                "start_time": stage_result.metadata.start_time.isoformat()
                if stage_result.metadata.start_time
                else None,
                "end_time": stage_result.metadata.end_time.isoformat()
                if stage_result.metadata.end_time
                else None,
            ...
            "context": {
                ...
                "total_stages_completed": len(
                    [r for r in context.stage_results.values() if r.success]
                ),
                "total_stages_failed": len(
                    [r for r in context.stage_results.values() if r.failed]
                ),
            },
            "metadata": {
    ...
    async def notify_pipeline_completion(
        ...
        logger.info(
            "Sending pipeline completion webhook",
            webhook_url=webhook_url,
            success=success,
        )
        ...
        for attempt in range(self.max_retries + 1):
            try:
                async with httpx.AsyncClient(timeout=self.timeout_seconds) as client:
                    ...
                    if response.status_code < 400:
                        logger.info(
                            "Pipeline webhook notification sent successfully",
                            webhook_url=webhook_url,
                            status_code=response.status_code,
                        )
                        return True
                        ...
                        logger.warning(
                            "Pipeline webhook notification failed",
                            webhook_url=webhook_url,
                            status_code=response.status_code,
                            response_text=response.text,
                        )
                ...
                logger.error(
                    "Pipeline webhook notification error",
                    webhook_url=webhook_url,
                    error=str(e),
                    attempt=attempt + 1,
                )
    ...
    def _create_pipeline_webhook_payload(
        self, context: StageContext, success: bool, error_message: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        total_execution_time = sum(
            r.metadata.execution_time for r in context.stage_results.values()
        )
