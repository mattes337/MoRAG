...
class StageManager:
    ...
    def _get_mock_output_file(
        self, stage_type: StageType, input_files: List[Path], context: StageContext
    ) -> Optional[Path]:
    ...
    def _detect_content_type(
        ...
        if source_path and "youtube.com" in str(source_path):
            return "youtube"
        elif source_path and any(
            domain in str(source_path) for domain in ["http://", "https://"]
        ):
            return "web"
    ...
    async def execute_stage(
        ...
        if context.resume_from_existing:
            ...
            if all(output.exists() for output in expected_outputs):
                logger.info(
                    "Skipping stage - outputs already exist",
                    stage_type=stage_type.value,
                    outputs=[str(f) for f in expected_outputs],
                )
        ...
        try:
            ...
            if not stage.validate_inputs(input_files):
                ...
                logger.error(
                    "Input validation failed - detailed info",
                    stage_type=stage_type.value,
                    validation_details=validation_details,
                )

                # Create more descriptive error message based on stage type
                error_message = self._create_validation_error_message(
                    stage_type, input_files, validation_details
                )
            ...
            logger.error(
                "Input validation failed with exception",
                stage_type=stage_type.value,
                error=str(e),
        ...
        try:
            ...
            if timeout:
                result = await asyncio.wait_for(
                    stage.execute(input_files, context), timeout=timeout
                )
            else:
            ...
            raise StageTimeoutError(
                error_msg, stage_type=stage_type.value, timeout_seconds=timeout
            )
            ...
            raise StageExecutionError(
                error_msg, stage_type=stage_type.value, original_error=e
            )
    ...
    async def execute_stage_chain(
        ...
        """
        logger.info(
            "Starting stage chain execution", stages=[s.value for s in stage_types]
        )
        ...
        for stage_type in ordered_stages:
            try:
                ...
                if result.success or result.skipped:
                    ...
                    logger.error(
                        "Required stage failed, stopping chain",
                        stage_type=stage_type.value,
                    )
                    break
                ...
                if stage.is_optional():
                    logger.warning(
                        "Optional stage failed, continuing chain",
                        stage_type=stage_type.value,
                    )
                    continue
                else:
        ...
        logger.info(
            "Stage chain execution completed",
    ...
    def _create_validation_error_message(
        ...
        if stage_type == StageType.FACT_GENERATOR:
            ...
            if file.suffix.lower() not in supported_extensions and not str(
                file
            ...
            if file_count < 1:
                return (
                    f"Ingestor stage requires at least 1 input file, got {file_count}."
                )
