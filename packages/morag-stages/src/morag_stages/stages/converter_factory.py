...
class ConverterFactory:
    ...
    async def initialize(self):
        ...
        try:
            if SERVICES_AVAILABLE:
                ...
                logger.warning(
                    "MoRAG services not available, using fallback converters"
                )
    ...
    def should_use_markitdown(
        ...
        try:
            ...
            logger.warning(
                "Error determining markitdown usage", file=str(file_path), error=str(e)
            )
    ...
    def validate_conversion_quality(
        ...
        try:
            if not content or len(content.strip()) < 10:
                logger.warning(
                    "Conversion produced very short content", file=str(file_path)
                )
                return False
            ...
            logger.warning(
                "Error validating conversion quality", file=str(file_path), error=str(e)
            )
    ...
    def get_converter_type(
        ...
        try:
            ...
            if "youtube" in content_type_str:
                ...
                doc_type in content_type_str
                for doc_type in ["document", "pdf", "doc", "txt"]
            ):
                return "markitdown"
            else:
            ...
            logger.warning(
                "Error determining converter type", file=str(file_path), error=str(e)
            )
            return "markitdown"
