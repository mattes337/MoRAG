...
class ConversionProcessors:
    ...
    async def process_file(
        ...
        try:
            ...
            converter_type = self.converter_factory.get_converter_type(
            ...
            return {
                "success": False,
                "error": str(e),
                "metadata": {"processing_time": 0, "processor_type": "error"},
            }
    ...
    async def _process_with_markitdown(
        ...
        try:
            ...
            if not self.converter_factory.validate_conversion_quality(
                content, input_file
            ):
                logger.warning(
                    "Conversion quality validation failed", file=str(input_file)
                )
                return {
                    "success": False,
            ...
            processing_time = (datetime.now() - start_time).total_seconds()
            logger.error(
                "Markitdown processing failed", file=str(input_file), error=str(e)
            )
            return {
                "success": False,
    ...
    async def _process_youtube(
        ...
        try:
            ...
            processing_time = (datetime.now() - start_time).total_seconds()
            logger.error(
                "YouTube processing failed", file=str(input_file), error=str(e)
            )
            return {
                "success": False,
    ...
    async def _process_with_services(
        ...
        try:
            ...
            processing_time = (datetime.now() - start_time).total_seconds()
            logger.error(
                "Services processing failed", file=str(input_file), error=str(e)
            )
            return {
                "success": False,
