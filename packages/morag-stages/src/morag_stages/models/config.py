...
class AgentModelConfig(BaseModel):
    ...
    default_model: Optional[str] = Field(
        default=None, description="Default LLM model for all agents"
    )
    agent_models: Dict[str, str] = Field(
        default_factory=dict, description="Agent-specific model overrides"
    )
...
class StageConfig(BaseModel):
    ...
    timeout_seconds: Optional[int] = Field(
        default=None, description="Timeout for stage execution"
    )
    ...
    agent_model_config: Optional[AgentModelConfig] = Field(
        default=None, description="Agent model configuration"
    )
...
class MarkdownConversionConfig(StageConfig):
    ...
    include_timestamps: bool = Field(
        default=True, description="Include timestamps in transcription"
    )
    transcription_model: str = Field(
        default="whisper-large", description="Transcription model to use"
    )
    speaker_diarization: bool = Field(
        default=True, description="Enable speaker diarization"
    )
    topic_segmentation: bool = Field(
        default=True, description="Enable topic segmentation"
    )

    # Document processing
    chunk_on_sentences: bool = Field(
        default=True, description="Chunk documents on sentence boundaries"
    )
    ...
    generate_thumbnails: bool = Field(
        default=False, description="Generate video thumbnails"
    )
    thumbnail_interval: int = Field(
        default=60, description="Thumbnail interval in seconds"
    )
...
class MarkdownOptimizerConfig(StageConfig):
    ...
    improve_readability: bool = Field(
        default=True, description="Improve text readability"
    )
    preserve_timestamps: bool = Field(
        default=True, description="Preserve timestamp information"
    )
    enhance_structure: bool = Field(
        default=True, description="Enhance document structure"
    )
...
class ChunkerConfig(StageConfig):
    ...
    strategy: str = Field(
        default="semantic", description="Chunking strategy (semantic, page, topic)"
    )
    ...
    respect_boundaries: bool = Field(
        default=True, description="Respect document boundaries (pages, sections)"
    )
...
class FactGeneratorConfig(StageConfig):
    ...
    max_facts_per_chunk: int = Field(
        default=10, description="Maximum facts to extract per chunk"
    )
    confidence_threshold: float = Field(
        default=0.3, description="Minimum confidence for fact extraction"
    )

    # Entity extraction
    extract_entities: bool = Field(
        default=True, description="Extract entities from facts"
    )
    entity_types: List[str] = Field(
        default_factory=list, description="Specific entity types to extract"
    )

    # Relation extraction
    extract_relations: bool = Field(
        default=True, description="Extract relations between entities"
    )
    relation_confidence: float = Field(
        default=0.4, description="Minimum confidence for relations"
    )
...
class IngestorConfig(StageConfig):
    ...
    databases: List[str] = Field(
        default_factory=lambda: ["qdrant", "neo4j"], description="Target databases"
    )
    collection_name: Optional[str] = Field(
        default=None, description="Collection/database name"
    )
    ...
    embedding_batch_size: int = Field(
        default=100, description="Batch size for embeddings"
    )

    # Validation
    validate_before_insert: bool = Field(
        default=True, description="Validate data before insertion"
    )
    skip_on_error: bool = Field(
        default=False, description="Skip items that cause errors"
    )
...
class PipelineConfig(BaseModel):
    ...
    markdown_conversion: MarkdownConversionConfig = Field(
        default_factory=MarkdownConversionConfig
    )
    markdown_optimizer: MarkdownOptimizerConfig = Field(
        default_factory=MarkdownOptimizerConfig
    )
    ...
    max_parallel_stages: int = Field(
        default=1, description="Maximum parallel stage execution"
    )
    resume_from_existing: bool = Field(
        default=True, description="Resume from existing files"
    )
