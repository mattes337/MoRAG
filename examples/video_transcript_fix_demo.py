...
def main():
    ...
    if "[0]" in improved_output and "[45]" in improved_output:
        improvements.append(
            "✅ Proper timestamps: [0] and [45] instead of [0] for all topics"
        )
    ...
    if (
        "Erklärung zum Fukuda-Test" in improved_output
        and "Physiotherapie und Behandlung" in improved_output
    ):
        improvements.append(
            "✅ Meaningful topic titles: Uses actual topic names when available"
        )
    ...
    print(
        f"   • Dialogue lines: {len([l for l in improved_output.split(chr(10)) if ': ' in l])}"
    )
    print(f"   • Topics: {improved_output.count('# ')}")
    print(
        f"   • Speakers detected: {len(set([l.split(':')[0] for l in improved_output.split(chr(10)) if ': ' in l]))}"
    )
