...
class TestVideoFFmpegErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_ffmpeg_error_stderr_capture(
        self, video_processor, temp_video_file, video_config
    ):
        ...
        mock_error = FFmpegError(
            "ffmpeg",
            "stdout",
            b'FFmpeg error: Invalid argument\nUnable to parse option value "None"',
        )
        mock_error.stderr = (
            b'FFmpeg error: Invalid argument\nUnable to parse option value "None"'
        )
        ...
        with patch(
            "morag.processors.video.ffmpeg_probe", return_value=mock_metadata
        ), patch("morag.processors.video.ffmpeg_run", side_effect=mock_error), patch(
    ...
    @pytest.mark.asyncio
    async def test_parameter_filtering_excludes_none_values(
        ...
        with patch(
            "morag.processors.video.ffmpeg_probe", return_value=mock_metadata
        ), patch("morag.processors.video.ffmpeg_run") as mock_run, patch(
            "morag.processors.video.ffmpeg_output"
        ) as mock_output, patch(
            "morag.processors.video.ffmpeg_input"
        ) as mock_input, patch(
            "pathlib.Path.exists", return_value=True
        ), patch(
            "pathlib.Path.stat"
    ...
    @pytest.mark.asyncio
    async def test_different_audio_formats_parameter_handling(
        self, video_processor, temp_video_file
    ):
    ...
    @pytest.mark.asyncio
    async def test_ffmpeg_quiet_false_for_error_capture(
        self, video_processor, temp_video_file
    ):
    ...
    def test_parameter_filtering_logic(self):
        ...
        old_params = {
            ...
            "audio_bitrate": "128k"
            if "wav" == "mp3"
            else None,  # This would be None for WAV
        }
