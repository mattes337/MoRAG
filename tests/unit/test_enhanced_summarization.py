    ...
    with patch("morag.services.summarization.gemini_service") as mock_gemini:
        ...
        mock_gemini.generate_summary = AsyncMock(
            return_value=SummaryResult(
                summary="Test summary", token_count=10, model="gemini-2.0-flash-001"
            )
        )
...
class TestEnhancedSummarizationService:
    ...
    @pytest.mark.asyncio
    async def test_generate_summary_abstractive(
        self, summarization_service, mock_gemini_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_summary_hybrid(
        self, summarization_service, mock_gemini_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_summary_hierarchical(
        self, summarization_service, mock_gemini_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_summary_contextual(
        self, summarization_service, mock_gemini_service
    ):
        """Test contextual summary generation."""
        text = "This document discusses methodology and results of the research."
        config = SummaryConfig(
            strategy=SummaryStrategy.CONTEXTUAL, focus_areas=["methodology", "results"]
        )
    ...
    def test_create_adaptive_config(self, summarization_service):
        ...
        academic_text = (
            "This research study presents methodology and analysis of results."
        )
    ...
    def test_build_enhanced_prompt(self, summarization_service):
        ...
        )  # "detailed" becomes "comprehensive coverage"
    ...
    def test_get_type_specific_instruction(self, summarization_service):
        ...
        academic_instruction = summarization_service._get_type_specific_instruction(
            DocumentType.ACADEMIC
        )
        technical_instruction = summarization_service._get_type_specific_instruction(
            DocumentType.TECHNICAL
        )
        business_instruction = summarization_service._get_type_specific_instruction(
...
class TestQualityAssessment:
    ...
    def test_assess_completeness(self, summarization_service):
        ...
        complete_score = summarization_service._assess_completeness(
            original, complete_summary
        )
        incomplete_score = summarization_service._assess_completeness(
            original, incomplete_summary
        )
    ...
    def test_assess_conciseness(self, summarization_service):
        ...
        perfect_score = summarization_service._assess_conciseness(
            perfect_summary, config
        )
        short_score = summarization_service._assess_conciseness(
            too_short_summary, config
        )
    ...
    def test_assess_relevance(self, summarization_service):
        ...
        relevant_score = summarization_service._assess_relevance(
            original, relevant_summary
        )
        irrelevant_score = summarization_service._assess_relevance(
            original, irrelevant_summary
        )
    ...
    def test_assess_readability(self, summarization_service):
        ...
        readable_summary = (
            "This is a clear and readable summary with appropriate sentence length."
        )
