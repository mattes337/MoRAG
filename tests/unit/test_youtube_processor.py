...
class TestYouTubeProcessor:
    ...
    @pytest.mark.asyncio
    async def test_process_url_metadata_only(
        self, youtube_processor, youtube_config, mock_metadata
    ):
        """Test processing URL for metadata only."""
        youtube_config.extract_metadata_only = True

        with patch.object(
            youtube_processor, "_extract_metadata_only", return_value=mock_metadata
        ):
            result = await youtube_processor.process_url(
                "https://youtube.com/watch?v=test", youtube_config
            )
    ...
    @pytest.mark.asyncio
    async def test_process_url_with_video_download(
        self, youtube_processor, youtube_config, mock_metadata, tmp_path
    ):
        ...
        with patch.object(
            ...
            youtube_processor, "_download_video", return_value=mock_video_path
        ), patch.object(
            youtube_processor, "_download_subtitles", return_value=[]
        ), patch.object(
            youtube_processor, "_download_thumbnails", return_value=[]
        ):
            result = await youtube_processor.process_url(
                "https://youtube.com/watch?v=test", youtube_config
            )
    ...
    @pytest.mark.asyncio
    async def test_process_url_with_audio_download(
        self, youtube_processor, youtube_config, mock_metadata, tmp_path
    ):
        ...
        with patch.object(
            ...
            youtube_processor, "_download_audio", return_value=mock_audio_path
        ), patch.object(
            youtube_processor, "_download_subtitles", return_value=[]
        ), patch.object(
            youtube_processor, "_download_thumbnails", return_value=[]
    ...
    async def test_extract_metadata_only_success(
        self, mock_yt_dlp, youtube_processor, mock_yt_dlp_info
    ):
        ...
        with patch("asyncio.to_thread", return_value=mock_yt_dlp_info):
            metadata = await youtube_processor._extract_metadata_only(
    ...
    async def test_extract_metadata_only_failure(self, mock_yt_dlp, youtube_processor):
        ...
        with pytest.raises(
            ExternalServiceError, match="YouTube metadata extraction failed"
        ):
            await youtube_processor._extract_metadata_only(
                "https://youtube.com/watch?v=test"
            )
    ...
    async def test_download_video_success(
        self, mock_yt_dlp, youtube_processor, youtube_config, tmp_path
    ):
        ...
        with patch("asyncio.to_thread"), patch(
            "pathlib.Path.glob", return_value=[mock_video_file]
        ):
            result = await youtube_processor._download_video(
                "https://youtube.com/watch?v=test", youtube_config
            )
    ...
    async def test_download_video_failure(
        self, mock_yt_dlp, youtube_processor, youtube_config
    ):
    ...
    async def test_download_audio_success(
        ...
        with patch("asyncio.to_thread"), patch(
            "pathlib.Path.glob", return_value=[mock_audio_file]
        ):
            result = await youtube_processor._download_audio(
                "https://youtube.com/watch?v=test", youtube_config
            )
    ...
    async def test_download_subtitles_success(
        ...
        with patch("asyncio.to_thread"), patch(
            "pathlib.Path.glob", side_effect=[mock_sub_files[:1], mock_sub_files[1:]]
        ):
            result = await youtube_processor._download_subtitles(
                "https://youtube.com/watch?v=test", youtube_config
            )
    ...
    @pytest.mark.asyncio
    @patch("yt_dlp.YoutubeDL")
    async def test_download_thumbnails_success(
        self, mock_yt_dlp, youtube_processor, youtube_config, tmp_path
    ):
        ...
        with patch("asyncio.to_thread"), patch(
            "pathlib.Path.glob",
            side_effect=[mock_thumb_files[:1], [], mock_thumb_files[1:]],
        ):
            result = await youtube_processor._download_thumbnails(
                "https://youtube.com/watch?v=test", youtube_config
            )
    ...
    @pytest.mark.asyncio
    @patch("yt_dlp.YoutubeDL")
    async def test_process_playlist_success(
        self, mock_yt_dlp, youtube_processor, youtube_config, mock_metadata
    ):
        ...
        with patch("asyncio.to_thread", return_value=mock_playlist_info), patch.object(
            youtube_processor,
            "process_url",
            return_value=YouTubeDownloadResult(
                video_path=None,
                audio_path=None,
                subtitle_paths=[],
                thumbnail_paths=[],
            ...
            results = await youtube_processor.process_playlist(
                "https://youtube.com/playlist?list=test", youtube_config
            )
    ...
    async def test_process_playlist_failure(self, youtube_processor, youtube_config):
        ...
        with patch("yt_dlp.YoutubeDL", side_effect=Exception("Playlist error")):
            with pytest.raises(ProcessingError, match="Playlist processing failed"):
                await youtube_processor.process_playlist(
                    "https://youtube.com/playlist?list=test", youtube_config
                )
    ...
    @pytest.mark.asyncio
    async def test_process_url_exception_handling(
        self, youtube_processor, youtube_config
    ):
        ...
        with patch.object(
            ...
            with pytest.raises(ProcessingError, match="YouTube processing failed"):
                await youtube_processor.process_url(
                    "https://youtube.com/watch?v=test", youtube_config
                )
