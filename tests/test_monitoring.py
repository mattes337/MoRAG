...
class TestLoggingService:
    ...
    def test_log_performance_metric(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch('morag.core.config.settings.log_file', f"{temp_dir}/test.log"):
                service = LoggingService()
                service.log_performance_metric("response_time", 0.5, "seconds", {"endpoint": "/api/test"})
...
class TestMetricsCollector:
    ...
    def test_system_metrics_collection(self, mock_net, mock_disk, mock_memory, mock_cpu, collector):
        """Test system metrics collection."""
        # Mock psutil responses
        mock_cpu.return_value = 25.5
        mock_memory.return_value = Mock(percent=60.0, used=8*1024*1024*1024, total=16*1024*1024*1024)
        mock_disk.return_value = Mock(used=100*1024*1024*1024, total=500*1024*1024*1024)
    ...
    async def test_application_metrics_collection(self, collector):
        ...
        with patch('morag.services.task_manager.task_manager') as mock_task_manager:
            ...
            assert metrics.queue_lengths == {'document_processing': 2, 'audio_processing': 1}
    ...
    def test_metrics_history_management(self, collector):
        ...
        for i in range(5):
            collector.metrics_history.append({'test': i, 'timestamp': datetime.utcnow().isoformat()})
            # Manually trigger trimming logic
            if len(collector.metrics_history) > collector.max_history_size:
                collector.metrics_history = collector.metrics_history[-collector.max_history_size:]
    ...
    async def test_metrics_endpoint(self):
        ...
        with patch.dict('sys.modules', {
            ...
            with patch('morag.core.config.settings.metrics_enabled', True):
                with patch('morag.services.metrics_service.metrics_collector') as mock_collector:
