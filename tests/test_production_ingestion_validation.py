...
class TestProductionIngestionValidation:
            ...
            "facts": [
                {
                    ...
                    "domain": "technology",
                    "structured_metadata": {
                        "primary_entities": ["Artificial intelligence", "computer science", "intelligent machines"]
                    },
    ...
    def test_confidence_distribution_validation(self, sample_facts_data):
        ...
        assert total_facts == len(facts), "Confidence distribution should account for all facts"
    ...
    def test_relationship_validation(self, sample_facts_data):
        ...
        assert stats['entity_to_entity'] >= 1, "Should identify entity-to-entity relationships"
        assert stats['fact_to_fact'] >= 1, "Should identify fact-to-fact relationships"

        # Check relationship types
        assert 'SUBSET_OF' in stats['relationship_types'], "Should track relationship types"
        assert 'RELATED_TO' in stats['relationship_types'], "Should track relationship types"
    ...
    def test_production_readiness_assessment(self, sample_facts_data):
        ...
        rel_validation = self._validate_relationships(sample_facts_data["relationships"])
        ...
        assert readiness in ["ready", "mostly_ready"], f"Production readiness: {readiness}, issues: {total_issues}"
    ...
    def _validate_facts(self, facts: List[Dict]) -> Dict[str, Any]:
        ...
        problematic_patterns = [
            r'\bit\b', r'\bthey\b', r'\bthem\b', r'\bthis\b', r'\bthese\b',
        ...
        for i, fact in enumerate(facts):
            ...
            has_pronouns = any(re.search(pattern, fact_text, re.IGNORECASE) for pattern in problematic_patterns)
            if not has_pronouns:
                stats['self_contained'] += 1
            else:
                issues.append(f"Fact {i+1}: Contains pronouns - '{fact.get('fact_text', '')[:100]}...'")
    ...
    def _validate_relationships(self, relationships: List[Dict]) -> Dict[str, Any]:
        ...
        for rel in relationships:
            ...
            is_entity_to_entity = (
                not source.startswith('fact_') and
                not target.startswith('fact_') and
                source and target
            )
    ...
    def _simulate_qdrant_ingestion(self, data: Dict) -> Dict[str, Any]:
        ...
        for fact in facts:
            chunk = {
                ...
                'metadata': {
                    'confidence': fact.get('confidence'),
                    'entities': fact.get('structured_metadata', {}).get('primary_entities', [])
                },
        ...
        return {
            ...
            'stats': {
                ...
                'avg_text_length': sum(len(chunk['text']) for chunk in vector_chunks) / len(vector_chunks) if vector_chunks else 0,
                'entities_per_chunk': sum(len(chunk['metadata']['entities']) for chunk in vector_chunks) / len(vector_chunks) if vector_chunks else 0
            }
        }
