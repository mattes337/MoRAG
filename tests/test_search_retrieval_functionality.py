...
class TestSearchRetrievalFunctionality:
    ...
    def test_search_filtering_capabilities(self, sample_vector_results):
        ...
        tech_results = [
            r
            for r in sample_vector_results
            if r["metadata"].get("domain") == "technology"
        ]
        assert len(tech_results) == len(
            sample_vector_results
        ), "All sample results should be technology domain"

        # Test fact type filtering
        definition_results = [
            r
            for r in sample_vector_results
            if r["metadata"].get("fact_type") == "definition"
        ]
        assert (
            len(definition_results) >= 2
        ), "Should have multiple definition-type results"
        ...
        ai_results = [
            ...
            for r in sample_vector_results
            if any(
                "artificial intelligence" in entity.lower()
    ...
    def test_graph_entity_structure(self, sample_graph_entities):
        ...
        for entity in sample_graph_entities:
            ...
            ), f"Entity type should be valid, got {entity['type']}"
    ...
    def test_reasoning_path_structure(self, sample_reasoning_paths):
        ...
        for path_data in sample_reasoning_paths:
            ...
            assert (
                "relevance_score" in path_data
            ), "Path data should contain relevance score"
            assert "reasoning" in path_data, "Path data should contain reasoning"

            path = path_data["path"]
            assert (
                len(path) >= 3
            ), "Path should have at least 3 elements (entity-relation-entity)"
            assert (
                len(path) % 2 == 1
            ), "Path should have odd number of elements (alternating entities and relations)"

            # Test relevance score
            score = path_data["relevance_score"]
            assert (
                0 <= score <= 1
            ), f"Relevance score should be between 0 and 1, got {score}"
    ...
    def test_search_query_processing(self):
        ...
        for query in test_queries:
            ...
            if query.strip():
                assert (
                    len(normalized) > 0
                ), f"Non-empty query should produce normalized result: {query}"
                assert (
                    normalized.lower() == normalized
                ), "Normalized query should be lowercase"
            else:
    ...
    def test_search_performance_thresholds(self, sample_vector_results):
        ...
        min_threshold = 0.5
        quality_results = [
            r for r in sample_vector_results if r["score"] >= min_threshold
        ]

        assert len(quality_results) == len(
            sample_vector_results
        ...
        assert len(unique_contents) == len(
            contents
        ), "Results should have diverse content"
    ...
    def test_hybrid_search_integration(
        self, sample_vector_results, sample_graph_entities
    ):
        ...
        assert (
            len(expandable_entities) >= 2
        ), "Graph should provide expansion for vector entities"
    ...
    def test_search_error_handling(self):
        ...
        invalid_queries = [
            ...
            "   ",
            "a" * 10000,
        ]  # None, empty, whitespace, too long
    ...
    def test_search_context_preservation(self, sample_vector_results):
        ...
        for result in sample_vector_results:
            ...
            if "entities" in metadata:
                ...
                assert all(
                    len(entity.strip()) > 0 for entity in entities
    ...
    def _safe_search(self, query: Any) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                {"id": "test", "score": 0.8, "metadata": {"content": "test result"}}
            ]
