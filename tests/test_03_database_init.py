...
class TestDatabaseInitialization:
    ...
    def test_init_script_exists(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "py_compile", str(script_path)], capture_output=True
        )
    ...
    async def test_init_script_logic(self, mock_service):
        ...
        mock_service.create_collection.assert_called_once_with(
            vector_size=768, force_recreate=False
        )
...
"""
        ...
        result = subprocess.run(
            [sys.executable, "-c", script_content], capture_output=True, text=True
        )
