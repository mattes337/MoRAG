...
class TestVideoPipelineIntegration:
    ...
    async def test_ffmpeg_service_integration(
        self, mock_exists, mock_to_thread, mock_probe, mock_video_file
    ):
    ...
    async def test_video_task_integration(
        self, mock_audio_task, mock_processor, mock_video_file
    ):
        ...
        assert (
            result["audio_processing_result"]["text"]
            == "This is transcribed audio content from the video."
        )
    ...
    async def test_keyframe_extraction_integration(
        self, mock_exists, mock_to_thread, mock_cv2_cap, mock_video_file
    ):
        ...
        with patch("cv2.cvtColor"), patch("cv2.calcHist"), patch(
            "cv2.compareHist", side_effect=[0.5, 0.2, 0.8, 0.1]
    ...
    @pytest.mark.asyncio
    @patch("morag.tasks.video_tasks.video_processor")
    async def test_video_without_audio_integration(
        self, mock_processor, mock_video_file
    ):
        ...
        result = await process_video_file(
            task_instance, str(mock_video_file), "test_task_id", process_audio=True
        )
