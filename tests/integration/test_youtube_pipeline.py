...
from morag_youtube import YouTubeConfig, YouTubeProcessor
...
class TestYouTubeIntegration:
    ...
    async def test_youtube_processor_with_mocked_services(
        self, mock_gemini_service, mock_qdrant_service, mock_youtube_metadata, tmp_path
    ):
        ...
        with patch("yt_dlp.YoutubeDL") as mock_yt_dlp:
            ...
            with patch("asyncio.to_thread", return_value=mock_info):
                result = await processor.process_url(
                    "https://youtube.com/watch?v=test_video_123", config
                )
    ...
    async def test_youtube_task_with_embedding_storage(
        ...
        with patch(
            "morag.tasks.youtube_tasks.youtube_processor"
        ) as mock_processor, patch(
            "morag.tasks.youtube_tasks.gemini_service", mock_gemini_service
        ), patch(
            "morag.tasks.youtube_tasks.qdrant_service", mock_qdrant_service
        ):
    ...
    @pytest.mark.asyncio
    async def test_youtube_error_handling(self, mock_celery_task, tmp_path):
        ...
        mock_celery_task.status_updates.append(
            {"status": "FAILURE", "error": "Network error"}
        )

        # Verify status updates were called
        assert len(mock_celery_task.status_updates) > 0
        assert any(
            update["status"] == "FAILURE" for update in mock_celery_task.status_updates
        )
    ...
    async def test_youtube_metadata_only_extraction(
        ...
        assert (
            ...
            == "This is a test video description for testing purposes."
        )
        assert result["metadata"]["uploader"] == "Test Channel"
        assert result["metadata"]["duration"] == 300
