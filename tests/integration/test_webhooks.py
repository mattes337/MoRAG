...
from src.morag.services.status_history import StatusEvent, status_history
...
def test_status_history_get_recent_events():
    ...
    with patch.object(status_history, "redis_client") as mock_redis:
        ...
        mock_redis.keys.return_value = [
            b"task_history:test-123",
            b"task_history:test-456",
        ]
...
async def test_task_manager_webhook_integration():
    ...
    with patch(
        "morag.services.webhook.webhook_service.send_task_started"
    ) as mock_webhook:
...
async def test_task_manager_completion_webhook():
    ...
    with patch(
        "morag.services.webhook.webhook_service.send_task_completed"
    ) as mock_webhook:
        mock_webhook.return_value = True
...
async def test_task_manager_failure_webhook():
    ...
    with patch(
        "morag.services.webhook.webhook_service.send_task_failed"
    ) as mock_webhook:
        mock_webhook.return_value = True
