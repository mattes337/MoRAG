...
class TestVideoAudioIntegration:
    ...
    async def test_video_processor_enhanced_audio_integration(
        self, mock_video_file, mock_video_metadata, mock_audio_result
    ):
        """Test that VideoProcessor correctly integrates enhanced audio processing."""

        with patch("morag.processors.video.ffmpeg_probe") as mock_probe, patch(
            "morag.processors.video.ffmpeg_run"
        ) as mock_ffmpeg, patch(
            "morag.processors.video.asyncio.to_thread"
        ) as mock_thread:
            ...
            mock_audio_processor.process_audio_file = AsyncMock(
            ...
            with patch.object(Path, "exists", return_value=True), patch.object(
                ...
                assert (
                    result.audio_processing_result.speaker_diarization.total_speakers
                    == 2
                )
                assert (
                    result.audio_processing_result.topic_segmentation.total_topics == 2
                )
