...
class TestWebProcessorContentConversionIntegration:
    ...
    def web_processor(self, chunking_service, content_converter):
        ...
        return WebProcessor(
            chunking_service=chunking_service, content_converter=content_converter
        )
...
def hello_world():
    ...
    @pytest.mark.asyncio
    async def test_web_processor_with_content_converter(
        self, web_processor, sample_html_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_content_converter_fallback(
        self, web_processor, sample_html_response
    ):
        """Test fallback to markdownify when ContentConverter fails."""
        # Mock ContentConverter to fail
        with patch.object(
            web_processor.content_converter, "html_to_markdown"
        ) as mock_convert:
    ...
    @pytest.mark.asyncio
    async def test_different_conversion_options(
        self, web_processor, sample_html_response
    ):
    ...
    @pytest.mark.asyncio
    async def test_content_converter_metadata_extraction(
        self, web_processor, sample_html_response
    ):
...
.class {
    color: red;
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            config = WebScrapingConfig(convert_to_markdown=True)
            result = await web_processor.process_url(
                "https://example.com/complex", config
            )
            ...
            assert (
                "spans 2 rows" in markdown or "spans 2 columns" in markdown
            )  # Merged cells
    ...
    async def test_performance_with_large_content(self, web_processor):
        ...
        large_content_parts = [
            "<!DOCTYPE html><html><head><title>Large Test</title></head><body><main>"
        ]

        # Add many paragraphs
        for i in range(100):
            large_content_parts.append(
                f"<p>This is paragraph {i} with some <strong>bold</strong> and <em>italic</em> text.</p>"
            )
        ...
        for i in range(50):
            large_content_parts.append(
                f"<tr><td>{i}</td><td>Item {i}</td><td>Description for item {i}</td></tr>"
            )
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            config = WebScrapingConfig(convert_to_markdown=True)
            result = await web_processor.process_url(
                "https://example.com/large", config
            )

            assert result.success is True
            assert result.processing_time < 10.0  # Should complete within 10 seconds
            assert (
                len(result.content.markdown_content) > 1000
