...
import pytest
from morag_document.tasks import _process_document_impl, process_document_task
    ...
    with patch("morag.tasks.document_tasks.gemini_service") as mock:
        ...
        mock.generate_summary = AsyncMock(
            return_value=SummaryResult(
                summary="Test summary", token_count=5, model="gemini-2.0-flash-001"
            )
        )
...
@pytest.mark.asyncio
async def test_document_processing_task_success(
    mock_gemini_service, mock_qdrant_service
):
...
Some content in section 2 with additional information.
    ...
    try:
        ...
        result = await _process_document_impl(
            mock_task, temp_path, "document", {"test": True, "source": "test"}
        )
...
@pytest.mark.asyncio
async def test_document_processing_with_docling(
    mock_gemini_service, mock_qdrant_service
):
    ...
    try:
        ...
        result = await _process_document_impl(
            mock_task, temp_path, "document", {"test": True}, True
        )
...
async def test_document_processing_file_not_found():
    ...
    with pytest.raises(Exception):
        await _process_document_impl(
            mock_task, "non_existent_file.pdf", "document", {"test": True}
        )
...
async def test_document_processing_summary_failure(mock_qdrant_service):
    ...
    try:
        ...
        with patch("morag.tasks.document_tasks.gemini_service") as mock_gemini:
            ...
            mock_gemini.generate_summary.side_effect = Exception(
                "Summary generation failed"
            )

            # Embedding generation succeeds
            mock_gemini.generate_embeddings_batch = AsyncMock(
                return_value=[
                    EmbeddingResult(
                        embedding=[0.1] * 768,
                        token_count=10,
            ...
            result = await _process_document_impl(
                mock_task, temp_path, "document", {"test": True}
            )
...
async def test_document_processing_metadata_preservation():
    ...
    try:
        with patch("morag.tasks.document_tasks.gemini_service") as mock_gemini, patch(
            "morag.tasks.document_tasks.qdrant_service"
        ) as mock_qdrant:
            ...
            result = await _process_document_impl(
                mock_task, temp_path, "document", original_metadata
            )
...
async def test_document_processing_progress_tracking():
    ...
    try:
        with patch("morag.tasks.document_tasks.gemini_service") as mock_gemini, patch(
            ...
            assert (
                len(progress_values) >= 4
            )  # At least validation, parsing, processing, completion
