...
class TestOpenIEPipelineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_graph_processor_with_openie(
        self, sample_document, mock_neo4j_config
    ):
        """Test graph processor with OpenIE integration."""
        try:
            from morag_services.graph_processor import (
            ...
            has_enhanced_builder = (
                hasattr(processor, "_enhanced_builder")
                and processor._enhanced_builder is not None
            )
            ...
            if has_enhanced_builder:
                ...
                processor._enhanced_builder.process_document = AsyncMock(
                    return_value=mock_result
                )

                # Test document processing
                result = await processor.process_document(
                    sample_document, document_path="test_document.txt"
                )
                ...
                logger.warning(
                    "⚠️  Enhanced graph builder not available, testing fallback mode"
                )
                ...
                if processor._entity_extractor and processor._relation_extractor:
                    ...
                    if processor._storage:
                        processor._storage.store_entities = AsyncMock(
                            return_value={"entities_stored": 0}
                        )
                        processor._storage.store_relations = AsyncMock(
                            return_value={"relations_stored": 0}
                        )
        ...
        try:
            ...
            class MockStorage(BaseStorage):
                ...
                async def store_entities(self, entities):
                    return {"entities_stored": len(entities)}

                async def store_relations(self, relations):
                    return {"relations_stored": len(relations)}
            ...
            builder = EnhancedGraphBuilder(
                storage=mock_storage, enable_openie=True, openie_config=openie_config
            )

            # Check OpenIE availability
            openie_available = (
                hasattr(builder, "openie_enabled") and builder.openie_enabled
            )
            ...
            if openie_available:
                ...
                if hasattr(builder, "openie_extractor") and builder.openie_extractor:
                    builder.openie_extractor.extract_full = AsyncMock(
                ...
                builder._store_entities_and_relations = AsyncMock(
                    return_value=MagicMock(
                        entities_created=0,
                        relations_created=0,
                        entity_ids=[],
                ...
                if hasattr(builder, "cleanup_manager"):
                    builder.cleanup_manager.cleanup_document_data = AsyncMock(
                        return_value=MagicMock()
                    )
                ...
                try:
                    result = await builder.process_document(
                        sample_document, "test_document_001"
                    )
                    ...
                    logger.warning(
                        f"Enhanced graph builder processing failed (expected): {e}"
                    )
    ...
    async def test_openie_service_integration(self):
        ...
        try:
            ...
            if service.enabled:
                ...
                logger.info(
                    f"   - Confidence threshold: {service.confidence_threshold}"
                )
    ...
    async def test_openie_configuration_integration(self):
        ...
        try:
            ...
            if missing_configs:
                ...
                logger.info(
                    f"   - Confidence threshold: {settings.openie_confidence_threshold}"
                )
                logger.info(
                    f"   - Entity linking: {settings.openie_enable_entity_linking}"
                )
                logger.info(
                    f"   - Predicate normalization: {settings.openie_enable_predicate_normalization}"
                )
    ...
    async def test_end_to_end_pipeline(self, sample_document):
        ...
        results["enhanced_builder"] = await self.test_enhanced_graph_builder_direct(
            sample_document
        )

        # Test 4: Graph processor
        results["graph_processor"] = await self.test_graph_processor_with_openie(
            sample_document, {}
        )
