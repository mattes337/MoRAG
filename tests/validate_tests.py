...
class TestValidator:
    ...
    def validate_file(self, file_path: Path) -> Dict[str, Any]:
        ...
        result = {"file": str(file_path), "valid": True, "issues": []}
        ...
        if not syntax_valid:
            ...
            self.validation_results["syntax_errors"].append(
                {"file": str(file_path), "error": syntax_msg}
            )
        ...
        if structure_issues:
            result["issues"].extend(
                [f"Structure: {issue}" for issue in structure_issues]
            )
    ...
    def print_summary(self):
        ...
        success_rate = (
            (results["valid_files"] / results["total_files"]) * 100
...
def main():
    ...
    success_rate = (
        validator.validation_results["valid_files"]
        / validator.validation_results["total_files"]
    ) * 100
